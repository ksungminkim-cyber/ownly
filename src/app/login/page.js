...
export default function AuthPage() {
        ...
        const { error } = await supabase.auth.signUp({
          ...
          options: {
            data: {
              full_name: form.name,
              phone: form.phone,
              nickname: autoNickname,  // ← 자동 생성 닉네임
            },
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", fontFamily:"'Pretendard','DM Sans',sans-serif", background:"#f5f4f0" }}>
      {/* 왼쪽 브랜드 패널 (데스크탑) */}
      <div style={{ display:"none", width:"45%", background:"#1a2744", position:"relative", overflow:"hidden",
        flexDirection:"column", justifyContent:"space-between", padding:"48px" }} className="login-brand-panel">
        <div style={{ position:"absolute", inset:0, opacity:0.05,
          backgroundImage:"radial-gradient(circle at 2px 2px, white 1px, transparent 0)", backgroundSize:"32px 32px" }} />
        <div style={{ position:"relative" }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:10, marginBottom:60 }}>
            <div style={{ width:40, height:40, borderRadius:12, background:"rgba(255,255,255,0.15)", backdropFilter:"blur(10px)",
        ...
        <div style={{ width:420, maxWidth:"100%" }}>
          ...
          <div style={{ textAlign:"center", marginBottom:32 }}>
            <div onClick={() => router.push("/")} style={{ display:"inline-flex", alignItems:"center", gap:10, cursor:"pointer", marginBottom:20 }}>
              <div style={{ width:44, height:44, borderRadius:13, background:"linear-gradient(145deg, #1a2744, #2d4270)",
                display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 6px 24px rgba(26,39,68,0.3)" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:22 }}>
            <button onClick={() => socialLogin("google")} disabled={!!socialLoading}
              style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, padding:"13px", borderRadius:14,
                background:"#fff", border:"1.5px solid #e8e6e0", color:"#333", fontWeight:700, fontSize:14, cursor:"pointer",
                opacity: socialLoading==="google" ? 0.7 : 1, boxShadow:"0 1px 4px rgba(26,39,68,0.06)", transition:"all .15s" }}>
            ...
            <button onClick={() => socialLogin("kakao")} disabled={!!socialLoading}
              style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, padding:"13px", borderRadius:14,
                background:"#FEE500", border:"1.5px solid #FEE500", color:"#191600", fontWeight:700, fontSize:14, cursor:"pointer",
                opacity: socialLoading==="kakao" ? 0.7 : 1, transition:"all .15s" }}>
            ...
            <button onClick={() => { setSocialLoading("naver"); window.location.href = "/api/auth/naver"; }} disabled={!!socialLoading}
              style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, padding:"13px", borderRadius:14,
                background:"#03C75A", border:"1.5px solid #03C75A", color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer",
                opacity: socialLoading==="naver" ? 0.7 : 1, transition:"all .15s" }}>
          ...
          {/* 탭 */}
          <div style={{ display:"flex", marginBottom:22, borderRadius:12, overflow:"hidden",
            border:"1.5px solid #e8e6e0", background:"#f8f7f4", padding:3, gap:3 }}>
            {["login","signup"].map(t => (
              <button key={t} onClick={() => { setTab(t); setErrors({}); setMsg(""); }}
                style={{ flex:1, padding:"9px", borderRadius:9, background: tab===t ? "#fff" : "transparent",
                  border:"none", color: tab===t ? "#1a2744" : "#8a8a9a", fontWeight:700, fontSize:13, cursor:"pointer",
                  transition:"all .15s", boxShadow: tab===t ? "0 1px 4px rgba(26,39,68,0.1)" : "none" }}>
                {t === "login" ? "로그인" : "회원가입"}
              </button>
            ))}
          </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {msg === "sent" && (
              <div style={{ background:"linear-gradient(135deg,#f0fdf8,#e6faf3)", border:"1.5px solid #0fa57340",
                borderRadius:14, padding:"18px 16px", display:"flex", flexDirection:"column", gap:10 }}>
                ...
                <button onClick={() => { setMsg(""); setTab("login"); }}
                  style={{ background:"none", border:"none", color:"#0fa573", fontSize:12, fontWeight:700,
                    cursor:"pointer", textDecoration:"underline", textAlign:"center", padding:0 }}>
                  로그인 화면으로 돌아가기
                </button>
              </div>
            )}
            ...
            <button onClick={submit} disabled={loading}
              style={{ padding:"14px", borderRadius:14, background:"linear-gradient(135deg,#1a2744,#2d4270)",
                border:"none", color:"#fff", fontWeight:800, fontSize:15, cursor:"pointer", marginTop:4,
                opacity: loading ? 0.7 : 1, boxShadow:"0 4px 20px rgba(26,39,68,0.3)", transition:"all .15s" }}>
              {loading ? <Spinner /> : tab === "login" ? "로그인" : "가입하기"}
            </button>

            {tab === "login" && (
              <button onClick={resetPassword} disabled={loading}
                style={{ background:"none", border:"none", color:"#8a8a9a", fontSize:12,
