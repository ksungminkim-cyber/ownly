                ...
                <div style={{ padding:"18px 22px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:12 }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:7, alignItems:"stretch", minWidth:140 }}>
                      ...
                      <button onClick={()=>{ try { localStorage.setItem("ownly_vacancy_flyer", JSON.stringify({ addr: gf(v,"addr","address")||"", pType: gf(v,"p_type","pType")||"", sub: gf(v,"sub_type","sub")||"", rent: Number(gf(v,"expected_rent","expectedRent")||0), dep: Number(gf(v,"deposit","dep")||0), maintenance: Number(v.maintenance||0), note: v.note||"" })); router.push("/dashboard/vacancy/flyer"); } catch { toast("안내문 생성 실패","error"); } }} style={{ padding:"8px 14px", borderRadius:9, fontSize:12, fontWeight:700, cursor:"pointer", border:"1px solid rgba(201,146,10,0.4)", background:"rgba(201,146,10,0.07)", color:"#c9920a" }}>🖨️ 중개사무소 안내문</button>
