    ...
    <div className="page-in page-padding" style={{ maxWidth: 960 }}>
      ...
      {vacancies.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          {vacancies.map((v) => (
            <div key={v.id} className="hover-lift" style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "17px 20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ display: "flex", gap: 7, marginBottom: 5 }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, color: C.rose, background: C.rose + "18", padding: "2px 7px", borderRadius: 5 }}>공실 D+{vacantDays(v.vacantSince)}</span>
                  </div>
                  <p style={{ fontSize: 14, fontWeight: 700, color: C.text }}>서울 {v.addr}</p>
                  <p style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>공실 시작: {v.vacantSince} · 기대 월세: {v.expectedRent}만원</p>
                  {v.note && <p style={{ fontSize: 11, color: C.indigo, marginTop: 3 }}>{v.note}</p>}
                </div>
                <button onClick={() => removeVacancy(v.id)} style={{ padding: "7px 14px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", border: `1px solid ${C.emerald}40`, background: C.emerald + "18", color: C.emerald }}>임대 완료</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)}>
        <h2 style={{ fontSize: 19, fontWeight: 800, color: "#fff", marginBottom: 16 }}>공실 등록</h2>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", gap: 10 }}>
            {["주거", "상가"].map((t) => (
              <button key={t} onClick={() => setForm((f) => ({ ...f, pType: t, sub: t === "주거" ? "아파트" : "1층 상가" }))} style={{ flex: 1, padding: "11px", borderRadius: 10, border: `2px solid ${form.pType === t ? C.indigo : C.border}`, background: form.pType === t ? C.indigo + "18" : "transparent", color: form.pType === t ? C.indigo : C.muted, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                {t === "주거" ? "🏠 주거" : "🏪 상가"}
              </button>
            ))}
          </div>
          ...
          <div>
            <p style={{ fontSize: 11, color: C.muted, fontWeight: 700, letterSpacing: ".5px", textTransform: "uppercase", marginBottom: 7 }}>메모</p>
            <textarea value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} placeholder="공실 관련 메모..." rows={2} style={{ width: "100%", padding: "11px 13px", fontSize: 13, color: C.text, background: C.faint, border: `1px solid ${C.border}`, borderRadius: 10, resize: "vertical", outline: "none" }} />
          </div>
          <div style={{ display: "flex", gap: 10 }}>
