"use client"; import { useState, useMemo } from "react"; import { useRouter } from "next/navigation"; import { SectionLabel, EmptyState, Modal, AuthInput, toast, ConfirmDialog } from "../../../components/shared"; import { C, COLORS } from "../../../lib/constants"; import { useApp } from "../../../context/AppContext"; import PlanGate from "../../../components/PlanGate"; import AddressInput from "../../../components/AddressInput"; const TYPE_CONFIG = { 주거: { icon: "🏠", color: C.indigo, subs: ["아파트","빌라","오피스텔","단독주택","원룸","투룸"] }, 상가: { icon: "🏪", color: C.amber, subs: ["1층 상가","집합상가","근린상가","오피스"] }, 오피스텔: { icon: "🏢", color: C.purple, subs: ["오피스텔(주거)","오피스텔(업무)"] }, 토지: { icon: "🌳", color: "#0d9488", subs: ["나대지","농지","임야","대지"] }, }; export default function VacancyPage() { return <PlanGate feature="vacancy"><VacancyContent /></PlanGate>; } function VacancyContent() { const router = useRouter(); const { tenants, vacancies, addVacancy, deleteVacancy, updateTenant } = useApp(); const [showModal, setShowModal] = useState(false); const [saving, setSaving] = useState(false); const [filterType, setFilterType] = useState("전체");

  // ✅ B안: 임대완료 확인 다이얼로그 상태
  const [completeTarget, setCompleteTarget] = useState(null);

  const initForm = () => ({ addr: "", pType: "주거", sub: "아파트", vacantSince: new Date().toISOString().slice(0,10), expectedRent: "", dep: "", maintenance: "", note: "" }); const [form, setForm] = useState(initForm()); const set = (k) => (val) => setForm(f => ({ ...f, [k]: val }));
  ...
  const allVacancies = useMemo(() => { const dedupAddrs = new Set(vacancies.map(v => v.addr)); const newTenantVac = tenantVacancies.filter(tv => !dedupAddrs.has(tv.addr)); return [...vacancies, ...newTenantVac]; }, [vacancies, tenantVacancies]);
  ...
  const cumulativeLoss = allVacancies.reduce((s, v) => { const rent = Number(v.expected_rent || 0); const since = v.vacant_since || new Date().toISOString().slice(0, 10); const months = Math.max(0, (new Date() - new Date(since)) / (1000 * 60 * 60 * 24 * 30.44)); return s + rent * months; }, 0);
  ...
  // ✅ B안: 임대완료 처리 후 물건 관리로 이동
  const handleComplete = async () => {
    const v = completeTarget;
    if (!v) return;
    try {
      if (v._source === "tenant") {
  ...
  const getField = (v, ...keys) => { for (const k of keys) if (v[k] !== undefined && v[k] !== null) return v[k]; return ""; };
  const filtered = filterType === "전체" ? allVacancies : allVacancies.filter(v => (getField(v, "p_type", "pType") || "주거") === filterType);
  const showMaint = form.pType === "상가" || form.pType === "오피스텔";
  const totalRent = (Number(form.expectedRent || 0) + Number(form.maintenance || 0));

  return ( <div className="page-in page-padding" style={{ maxWidth: 960 }}> <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:22, flexWrap:"wrap", gap:12 }}> <div> <SectionLabel>VACANCY MANAGEMENT</SectionLabel> <h1 style={{ fontSize:24, fontWeight:800, color:"#1a2744" }}>공실 관리</h1> <p style={{ fontSize:13, color:"#8a8a9a", marginTop:3 }}>전체 {totalUnits}실 중 공실 {allVacancies.length}실 {tenantVacancies.length > 0 && <span style={{ fontSize:11, color:"#5b4fcf", marginLeft:6, fontWeight:600 }}>물건 관리 연동 {tenantVacancies.length}건 포함</span>} </p> </div> <button onClick={() => setShowModal(true)} className="btn-primary" style={{ padding:"10px 20px", borderRadius:11, background:`linear-gradient(135deg,${C.indigo},${C.purple})`, border:"none", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer" }}>+ 공실 등록</button> </div>
    ...
    {allVacancies.length === 0 ? ( <EmptyState icon="✅" title="공실이 없습니다" desc="모든 호실이 임대 중입니다" action="공실 등록" onAction={() => setShowModal(true)} /> ) : filtered.length === 0 ? ( <EmptyState icon="🔍" title="해당 유형의 공실이 없습니다" desc="다른 유형 필터를 선택해보세요" /> ) : (
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {filtered.map(v => {
          ...
          return ( <div key={v.id} className="hover-lift" style={{ background:"#fff", border:`1px solid ${urgency.c}22`, borderRadius:16, padding:"18px 22px" }}> <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:12 }}> <div style={{ flex:1 }}> <div style={{ display:"flex", gap:6, marginBottom:8, flexWrap:"wrap" }}> <span style={{ fontSize:10, fontWeight:700, color:cfg.color, background:cfg.color+"18", padding:"2px 8px", borderRadius:5 }}>{cfg.icon} {sub || pType}</span> <span style={{ fontSize:10, fontWeight:700, color:urgency.c, background:urgency.c+"15", padding:"2px 8px", borderRadius:5 }}>{urgency.label} D+{days}</span> {fromTenant && <span style={{ fontSize:10, fontWeight:700, color:"#5b4fcf", background:"rgba(91,79,207,0.1)", padding:"2px 8px", borderRadius:5 }}>🔗 물건 관리 연동</span>} {days >= 90 && <span style={{ fontSize:10, fontWeight:700, color:"#fff", background:C.rose, padding:"2px 8px", borderRadius:5 }}>누적손실 {cardCumulativeLoss.toLocaleString()}만원</span>} </div>
              <p style={{ fontSize:15, fontWeight:700, color:"#1a2744", marginBottom:5 }}>{addr}</p>
              <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}> {rent > 0 && <span style={{ fontSize:12, color:"#8a8a9a" }}>💰 기대 월세 <b style={{ color:"#1a2744" }}>{rent.toLocaleString()}만원</b></span>} {dep > 0 && <span style={{ fontSize:12, color:"#8a8a9a" }}>🏦 보증금 <b style={{ color:"#1a2744" }}>{dep.toLocaleString()}만원</b></span>} {maint > 0 && <span style={{ fontSize:12, color:"#8a8a9a" }}>🏢 관리비 <b style={{ color:"#1a2744" }}>{maint.toLocaleString()}만원</b></span>} {rent+maint > 0 && <span style={{ fontSize:12, color:C.emerald, fontWeight:700 }}>총 {(rent+maint).toLocaleString()}만원/월</span>} </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8, alignItems:"flex-end" }}>
              {/* ✅ B안: 직접 처리 대신 확인 다이얼로그 열기 */}
              <button onClick={() => setCompleteTarget(v)} style={{ padding:"8px 16px", borderRadius:9, fontSize:12, fontWeight:700, cursor:"pointer", border:`1px solid ${C.emerald}40`, background:C.emerald+"18", color:"#0fa573", whiteSpace:"nowrap" }}>✅ 임대 완료</button>
    ...
    {/* ✅ B안: 임대완료 확인 다이얼로그 — 처리 후 물건 관리로 이동 안내 */}
    <ConfirmDialog
      open={!!completeTarget}
      title="임대 완료 처리"
      desc={completeTarget ? `${getField(completeTarget, "addr", "address")} 공실을 임대 완료 처리합니다.

처리 후 물건 관리 페이지로 이동해서 새 세입자 정보를 입력하시겠습니까?` : ""}
