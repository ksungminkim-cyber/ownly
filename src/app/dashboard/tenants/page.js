...
import { Badge, SectionLabel, SearchBox, EmptyState, Modal, toast } from "../../../components/shared";
import { C, STATUS_MAP, INTENT_MAP, daysLeft } from "../../../lib/constants";
import { useApp } from "../../../context/AppContext";

export default function TenantsPage() {
  const router = useRouter();
  const { tenants, updateTenantContacts, updateTenantIntent } = useApp();
  ...
  const filtered = useMemo(() => {
    let list = [...tenants];
    if      (filter === "만료임박") list = list.filter((t) => daysLeft(getEnd(t)) <= 90);
    else if (filter === "미납")     list = list.filter((t) => t.status === "미납");
    else if (filter !== "전체")     list = list.filter((t) => t.pType === filter);
    if (search) {
  ...
  const addContact = async () => {
    ...
    setSaving(true);
    try {
      const newContact = { date: new Date().toISOString().slice(0, 10), type: contactNote.type, note: contactNote.note };
      const updatedContacts = [newContact, ...(sel.contacts || [])];
      await updateTenantContacts(sel.id, updatedContacts);
