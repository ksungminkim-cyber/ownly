...
export default function TenantsPage() { const router = useRouter(); const { tenants, payments, repairs, updateTenantContacts, updateTenantIntent, loading, user } = useApp(); const [selected, setSelected] = useState(null); const [filter, setFilter] = useState("전체"); const [search, setSearch] = useState(""); const [showContact, setShowContact] = useState(false); const [contactNote, setContactNote] = useState({ type: "납부확인", note: "" }); const [saving, setSaving] = useState(false);
  ...
  if (loading) return <SkeletonTable rows={6} cols={3} />; /* 훅 선언 뒤로 이동 — 훅 개수가 렌더마다 달라지는 위반 제거 */ return ( <div className="page-in tenant-split" style={{ display: "flex", height: "100%", minHeight: "calc(100vh - 0px)" }}> <div className="tenant-list-panel" style={{ width: 310, borderRight: "1px solid #ebe9e3", display: "flex", flexDirection: "column", flexShrink: 0 }}> <div style={{ padding: "26px 18px 14px", borderBottom: "1px solid var(--border)" }}> <SectionLabel>TENANT MANAGEMENT</SectionLabel> <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}><h1 style={{ fontSize: 20, fontWeight: 800, color: "var(--text)" }}>세입자 관리</h1><button onClick={() => router.push("/dashboard/tenants/renewals")} className="btn btn-soft btn-sm">📅 갱신 일괄 관리 →</button></div> <div style={{ marginTop: 10 }}><SearchBox value={search} onChange={setSearch} placeholder="이름, 주소 검색..." /></div> </div> <div style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)", display: "flex", gap: 5, flexWrap: "wrap" }}> {["전체", "주거", "상가", "토지", "만료임박", "미납"].map((f) => ( <button key={f} onClick={() => setFilter(f)} className={`chip ${filter === f ? "is-active" : ""}`} style={{ padding: "4px 10px", fontSize: 11 }}>{f}</button> ))} </div> <div style={{ flex: 1, overflowY: "auto" }}> {filtered.length === 0 ? ( <div style={{ padding: 24, textAlign: "center" }}> <div style={{ fontSize: 32, marginBottom: 10 }}>{search ? "🔍" : tenants.length === 0 ? "🏠" : "👤"}</div> <p style={{ fontSize: 13, fontWeight: 700, color: "#1a2744", marginBottom: 6 }}>{search ? "검색 결과 없음" : tenants.length === 0 ? "등록된 물건이 없습니다" : "해당 조건의 세입자 없음"}</p> {tenants.length === 0 && <button onClick={() => router.push("/dashboard/properties")} style={{ padding: "8px 18px", borderRadius: 10, background: "#1a2744", color: "#fff", border: "none", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>물건 등록하러 가기 →</button>} </div> ) : filtered.map((t) => { const isSelected = sel?.id === t.id; const dl = daysLeft(getEnd(t)); return ( <div key={t.id} onClick={() => { setSelected(t); setDetailTab("contract"); }} style={{ padding: "13px 16px", borderBottom: "1px solid #ebe9e3", cursor: "pointer", borderLeft: "3px solid " + (isSelected ? C.indigo : "transparent"), background: isSelected ? "#f8f7f4" : "transparent", transition: "all .15s" }} onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = "#f5f4f0"; }} onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.background = "transparent"; }}> <div style={{ display: "flex", gap: 9, alignItems: "flex-start" }}> <div style={{ width: 34, height: 34, borderRadius: 10, background: (t.color || C.indigo) + "20", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: t.color || C.indigo, flexShrink: 0 }}>{getInitial(t.name)}</div> <div style={{ flex: 1, minWidth: 0 }}> <div style={{ display: "flex", justifyContent: "space-between" }}> <p style={{ fontSize: 13, fontWeight: 700, color: "#1a2744" }}>{t.name}</p> <Badge label={t.status} map={STATUS_MAP} /> </div> <p style={{ fontSize: 11, color: "#8a8a9a", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.sub} · {t.addr}</p>
                {/* ✅ ⑤ 전화번호 목록에서 바로 표시 */}
                {t.phone && (
        ...
        {tenants.length === 0 ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              {[ { l: "총 세입자", v: summaryStats.total + "명", c: "#1a2744", icon: "👤" }, { l: "월 총 수입", v: summaryStats.totalRent.toLocaleString() + "만원", c: C.emerald, icon: "💰" }, { l: "평균 월세", v: summaryStats.avgRent.toLocaleString() + "만원", c: C.indigo, icon: "📊" }, { l: "미납", v: summaryStats.unpaid + "건", c: summaryStats.unpaid > 0 ? C.rose : C.emerald, icon: summaryStats.unpaid > 0 ? "⚠️" : "✅" }, ].map(k => ( <div key={k.l} className="stat"> <p className="stat-label">{k.icon} {k.l}</p> <p className="num stat-value" style={{ color: k.c }}>{k.v}</p> </div> ))} </div>
      ...
      <div style={{ flex: 1, padding: "26px 30px", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ width: 46, height: 46, borderRadius: 13, background: (sel.color || C.indigo) + "20", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 19, fontWeight: 800, color: sel.color || C.indigo }}>{getInitial(sel.name)}</div>
            <div>
