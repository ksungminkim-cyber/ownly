...
export default function SettingsPage() {
  ...
  const saveNickname = async () => {
    if (!nickname.trim()) { toast("닉네임을 입력해주세요", "error"); return; }
    if (nickname.trim().length < 2 || nickname.trim().length > 20) {
      toast("닉네임은 2~20자 사이로 입력해주세요", "error"); return;
    }
    setSavingNick(true);
  ...
  const handleDelete = async () => {
    if (deleteInput !== "탈퇴합니다") {
      toast("확인 문구를 정확히 입력해주세요", "error"); return;
    }
    setDeleting(true);
    try {
    ...
    <div className="page-in page-padding" style={{ padding: "36px 40px", maxWidth: 720 }}>
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 20, marginBottom: 18 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ padding: "10px 13px", background: "var(--surface2)", borderRadius: 10, fontSize: 13, color: "var(--text-muted)", border: "1px solid var(--border)" }}>
            {user?.email || "—"}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 6 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            />
            <button onClick={randomNickname}
              style={{ padding: "10px 14px", borderRadius: 10, background: "var(--surface2)", border: "1px solid var(--border)", cursor: "pointer", fontSize: 13, color: "var(--text-muted)", whiteSpace: "nowrap" }}>
              🎲 랜덤
            </button>
            <button onClick={saveNickname} disabled={savingNick}
              style={{ padding: "10px 18px", borderRadius: 10, background: savingNick ? "#94a3b8" : "#1a2744", border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: savingNick ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}>
              {savingNick ? "저장 중..." : "저장"}
            </button>
          </div>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 20, marginBottom: 18 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: showPw ? 16 : 0 }}>
            ...
            <button onClick={() => setShowPw(o => !o)}
              style={{ fontSize: 12, color: "#1a2744", fontWeight: 700, background: "none", border: "none", cursor: "pointer" }}>
              {showPw ? "닫기 ▲" : "변경하기 ▼"}
            </button>
          </div>
          {showPw && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                { key: "next",    label: "새 비밀번호",      placeholder: "6자 이상" },
                { key: "confirm", label: "새 비밀번호 확인", placeholder: "한 번 더 입력" },
                ...
                <div key={key}>
                  ...
                  <input type="password" value={pwForm[key]} onChange={e => setPwForm(f => ({ ...f, [key]: e.target.value }))}
                    ...
                    style={{ width: "100%", padding: "10px 13px", fontSize: 13, color: "var(--text)", background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 10, outline: "none", boxSizing: "border-box" }} />
                </div>
              ))}
              <button onClick={savePassword} disabled={savingPw}
                style={{ padding: "11px", borderRadius: 10, background: savingPw ? "#94a3b8" : "#1a2744", border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: savingPw ? "not-allowed" : "pointer" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            ...
            <button key={label} onClick={action}
              style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 16px", borderRadius: 12, border: "1px solid var(--border)", background: "var(--surface2)", cursor: "pointer", textAlign: "left", width: "100%" }}
              onMouseEnter={e => { e.currentTarget.style.background = "var(--surface3)"; e.currentTarget.style.borderColor = "#1a2744"; }}
      ...
      <div style={{ background: "rgba(232,68,90,0.04)", border: "1px solid rgba(232,68,90,0.2)", borderRadius: 16, padding: 20 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, paddingBottom: 16, marginBottom: 16, borderBottom: "1px solid rgba(232,68,90,0.15)" }}>
          ...
          <button onClick={() => setConfirmReset(true)}
            style={{ padding: "9px 18px", borderRadius: 10, background: "transparent", border: "1px solid rgba(232,68,90,0.5)", color: "#e8445a", fontWeight: 700, fontSize: 13, cursor: "pointer", whiteSpace: "nowrap" }}>
            데이터 초기화
          </button>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          ...
          <button onClick={() => setConfirmDelete(true)}
            style={{ padding: "9px 18px", borderRadius: 10, background: "#e8445a", border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", whiteSpace: "nowrap" }}>
            회원 탈퇴
          </button>
        </div>
      </div>
      ...
      <Modal open={confirmReset} onClose={() => setConfirmReset(false)} width={400}>
        <div style={{ textAlign: "center", padding: "10px 0" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={handleReset} disabled={resetting} style={{ flex: 1, padding: 12, borderRadius: 11, background: "#e8445a", border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", opacity: resetting ? 0.7 : 1 }}>
        ...
        <div style={{ padding: "10px 0" }}>
          <div style={{ textAlign: "center", marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 13, color: "#8a8a9a", lineHeight: 1.7 }}>
              탈퇴하면 계정과 <strong>모든 데이터(물건·세입자·수금·계약)</strong>가<br />
              영구 삭제됩니다. 이 작업은 되돌릴 수 없습니다.
            </p>
          </div>
          <div style={{ marginBottom: 16 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#8a8a9a", marginBottom: 8 }}>
              확인을 위해 아래에 <strong style={{ color: "#e8445a" }}>탈퇴합니다</strong> 를 입력하세요
            </p>
            <input
              value={deleteInput}
              onChange={e => setDeleteInput(e.target.value)}
              placeholder="탈퇴합니다"
              style={{ width: "100%", padding: "11px 13px", fontSize: 13, color: "var(--text)", background: "var(--surface2)", border: `1.5px solid ${deleteInput === "탈퇴합니다" ? "#e8445a" : "var(--border)"}`, borderRadius: 10, outline: "none", boxSizing: "border-box" }}
            />
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => { setConfirmDelete(false); setDeleteInput(""); }}
              style={{ flex: 1, padding: 12, borderRadius: 11, background: "transparent", border: "1px solid var(--border)", color: "#8a8a9a", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>취소</button>
            <button onClick={handleDelete} disabled={deleting || deleteInput !== "탈퇴합니다"}
              style={{ flex: 1, padding: 12, borderRadius: 11, background: deleteInput === "탈퇴합니다" ? "#e8445a" : "#d1d5db", border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: deleteInput === "탈퇴합니다" && !deleting ? "pointer" : "not-allowed", opacity: deleting ? 0.7 : 1 }}>
              {deleting ? "탈퇴 중..." : "탈퇴 확인"}
            </button>
          </div>
        </div>
