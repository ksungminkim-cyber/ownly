...
export default function SettingsPage() {
  ...
  const handleDelete = async () => { if (deleteInput !== "탈퇴합니다") { toast("확인 문구를 정확히 입력해주세요", "error"); return; } setDeleting(true); try { const { data: { session } } = await supabase.auth.getSession(); const res = await fetch("/api/account/delete", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${session?.access_token || ""}` }, body: JSON.stringify({}) }); const data = await res.json(); if (!res.ok) throw new Error(data.error); await supabase.auth.signOut(); router.push("/login?msg=deleted"); } catch (err) { toast("탈퇴 실패: " + err.message, "error"); setDeleting(false); } };
