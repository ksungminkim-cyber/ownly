...
const PREMIUM_FEATURES = [
  ...
  { id: "kakao_alert",  href: "/dashboard/premium/kakao-alert",icon: "💬", title: "카카오 수금 알림", desc: "미납 세입자에게 자동 알림 발송",                       badge: "PRO",      badgeColor: "#c9920a", plan: "pro",  tag: null },
];
...
export default function DashboardPage() {
  ...
  const alerts = [
    ...tenants.filter((t) => t.status === "미납").map((t) => ({
      type: "danger", icon: "⚠️", text: t.name + "님 미납", sub: t.rent + "만원", page: "payments",
    })),
    ...tenants.filter((t) => daysLeft(t.end) <= 90 && t.status !== "미납").map((t) => ({
      type: "warn", icon: "📅", text: t.name + "님 만료 D-" + daysLeft(t.end), sub: t.addr, page: "tenants",
    })),
  ];
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 320px", minHeight: "100%", fontFamily: "'Pretendard','DM Sans',sans-serif", overflowX: "hidden", maxWidth: "100%" }}>

      {/* LEFT MAIN */}
      <div style={{ padding: isMobile ? "20px 16px" : "24px 20px 24px 28px", borderRight: isMobile ? "none" : "1px solid #ebe9e3", minHeight: "100vh", overflowX: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, gap: 8 }}>
          <div style={{ minWidth: 0 }}>
            ...
            <h1 style={{ fontSize: isMobile ? 20 : 24, fontWeight: 900, color: "#1a2744", letterSpacing: "-.5px", margin: 0 }}>대시보드</h1>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            <span style={{ fontSize: 12, color: "#8a8a9a", fontWeight: 600, whiteSpace: "nowrap" }}>{year}년 {month}월</span>
            <button onClick={() => router.push("/dashboard/properties")} style={{ padding: isMobile ? "7px 12px" : "8px 16px", borderRadius: 10, fontSize: 12, fontWeight: 700, background: "#1a2744", color: "#fff", border: "none", cursor: "pointer", whiteSpace: "nowrap" }}>
              + 물건 추가
            </button>
          </div>
        </div>
        ...
        {/* 시작 배너 */}
        {tenants.length === 0 && (
          <div style={{ background: "linear-gradient(135deg,rgba(26,39,68,0.04),rgba(15,165,115,0.04))", border: "1px solid #e0ede8", borderRadius: 14, padding: "18px 16px", marginBottom: 20 }}>
            <p style={{ fontSize: 11, fontWeight: 800, color: "#1a2744", marginBottom: 14 }}>🚀 시작하기 — 4단계로 임대 관리를 완성하세요</p>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap: 8 }}>
              {[
                ...
                <div key={step} onClick={() => router.push("/dashboard/" + page)}
                  style={{ background: "#fff", borderRadius: 10, padding: "12px", cursor: "pointer", border: "1px solid #ebe9e3" }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 10, marginBottom: 20 }}>
          {[
            ...
            <div key={k.l} onClick={() => router.push("/dashboard/" + k.p)}
              style={{ background: "#fff", border: "1px solid #ebe9e3", borderRadius: 14, padding: "16px", cursor: "pointer" }}
            ...
            >
              ...
              <p style={{ fontSize: 22, fontWeight: 900, color: "#1a2744", lineHeight: 1 }}>{k.v}</p>
        ...
        {/* 차트 */}
        <div style={{ background: "#fff", border: "1px solid #ebe9e3", borderRadius: 14, padding: "18px 16px", marginBottom: 16 }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #ebe9e3", borderRadius: 14, overflow: "hidden" }}>
          <div style={{ padding: "14px 16px 0", borderBottom: "1px solid #f0efe9" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: "#1a2744" }}>물건 목록</h3>
              <span onClick={() => router.push("/dashboard/properties")} style={{ fontSize: 11, color: "#8a8a9a", cursor: "pointer", fontWeight: 600, whiteSpace: "nowrap" }}>전체 보기 →</span>
            </div>
            <div style={{ display: "flex", gap: 4, overflowX: "auto", WebkitOverflowScrolling: "touch", paddingBottom: 2 }}>
              {FILTERS.map((f) => (
                <button key={f} onClick={() => { setFilter(f); setSelected(null); }}
                  style={{ padding: "5px 10px", borderRadius: "8px 8px 0 0", fontSize: 11, fontWeight: 700, cursor: "pointer", border: "none", background: filter === f ? "#1a2744" : "transparent", color: filter === f ? "#fff" : "#8a8a9a", whiteSpace: "nowrap", flexShrink: 0 }}
                >
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "64px 1fr 60px 48px" : "80px 1fr 70px 60px 70px 60px", padding: "8px 16px", background: "#faf9f6", borderBottom: "1px solid #f0efe9" }}>
            {(isMobile ? ["유형","주소 · 세입자","월세","상태"] : ["유형","주소 · 세입자","월세","보증금","잔여일","상태"]).map((h) => (
            ...
            : filtered.map((t) => {
                ...
                return (
                  <div key={t.id} onClick={() => setSelected(isSel ? null : t.id)}
                    style={{ display: "grid", gridTemplateColumns: isMobile ? "64px 1fr 60px 48px" : "80px 1fr 70px 60px 70px 60px", padding: "11px 16px", borderBottom: "1px solid #f4f3f0", cursor: "pointer", background: isSel ? "rgba(26,39,68,0.03)" : "transparent", borderLeft: isSel ? "2.5px solid #1a2744" : "2.5px solid transparent" }}
                    onMouseEnter={(e) => { if (!isSel) e.currentTarget.style.background = "#faf9f6"; }}
                    onMouseLeave={(e) => { if (!isSel) e.currentTarget.style.background = "transparent"; }}
                  >
                    <div>
                      <span style={{ fontSize: 10, fontWeight: 800, color: t.pType === "상가" ? "#e8960a" : t.pType === "토지" ? "#0d9488" : "#1a2744", background: t.pType === "상가" ? "rgba(232,150,10,0.1)" : t.pType === "토지" ? "rgba(13,148,136,0.1)" : "rgba(26,39,68,0.07)", padding: "3px 6px", borderRadius: 5, whiteSpace: "nowrap" }}>
                        {t.sub || t.pType}
                      </span>
                    </div>
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: "#1a2744", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.addr}</p>
                      <p style={{ fontSize: 11, color: "#a0a0b0", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</p>
                    </div>
                    <p style={{ fontSize: 12, fontWeight: 700, color: "#1a2744", alignSelf: "center", whiteSpace: "nowrap" }}>{Number(t.rent).toLocaleString()}만</p>
                    {!isMobile && <p style={{ fontSize: 12, color: "#8a8a9a", alignSelf: "center" }}>{t.dep ? Math.round(t.dep/100) + "백" : "-"}</p>}
                    {!isMobile && <p style={{ fontSize: 12, fontWeight: 700, color: dl <= 60 ? "#e8445a" : dl <= 90 ? "#e8960a" : "#8a8a9a", alignSelf: "center" }}>D-{dl}</p>}
                    <div style={{ alignSelf: "center" }}>
                      <span style={{ fontSize: 10, fontWeight: 800, color: ST[t.status]?.c || "#8a8a9a", background: ST[t.status]?.bg || "#f0efe9", padding: "3px 6px", borderRadius: 5, whiteSpace: "nowrap" }}>
                        {t.status}
                      </span>
                    </div>
                  </div>
      ...
      {/* RIGHT PANEL */}
      <div style={{ padding: isMobile ? "20px 16px" : "24px 20px", background: "#fff", display: "flex", flexDirection: "column", minHeight: isMobile ? "unset" : "100vh", overflowY: "auto", overflowX: "hidden", borderTop: isMobile ? "1px solid #ebe9e3" : "none" }}>
        {sel ? (
          <>
            <button onClick={() => setSelected(null)} style={{ fontSize: 11, color: "#8a8a9a", background: "none", border: "none", cursor: "pointer", marginBottom: 14, padding: 0, fontWeight: 600, textAlign: "left" }}>
              ← 목록으로
            </button>
            ...
            <div style={{ background: "#f8f7f4", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
              {[
                ["월세",   Number(sel.rent).toLocaleString() + "만원", "#0fa573"],
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                <button key={page} onClick={() => router.push("/dashboard/" + page)}
                  style={{ padding: "10px 14px", borderRadius: 10, background: bg, border: "none", color, fontWeight: 700, fontSize: 13, cursor: "pointer", textAlign: "left" }}
                >
                  {icon} {label}
                </button>
              ))}
          ...
          <>
            <div style={{ marginBottom: 22, background: "linear-gradient(135deg,#1a2744,#2d4270)", borderRadius: 16, padding: "20px" }}>
              ...
              {tenants.length === 0 ? (
                ...
                <div>
                  <p style={{ fontSize: 26, fontWeight: 900, color: "#fff", marginBottom: 6 }}>월 {totalRent.toLocaleString()}만원</p>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                {alerts.map((a, i) => (
                  <div key={i} onClick={() => router.push("/dashboard/" + a.page)}
                    style={{ display: "flex", gap: 10, padding: "10px 12px", borderRadius: 10, marginBottom: 7, cursor: "pointer", background: a.type === "danger" ? "rgba(232,68,90,0.05)" : "rgba(232,150,10,0.05)", border: "1px solid " + (a.type === "danger" ? "rgba(232,68,90,0.15)" : "rgba(232,150,10,0.15)") }}
                  >
                    <span style={{ fontSize: 14 }}>{a.icon}</span>
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: 700, color: "#1a2744" }}>{a.text}</p>
                      <p style={{ fontSize: 12, color: "#8a8a9a", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.sub}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                ...
                <span onClick={() => router.push("/dashboard/pricing")} style={{ fontSize: 11, color: "#5b4fcf", cursor: "pointer", fontWeight: 800, background: "rgba(91,79,207,0.08)", padding: "5px 12px", borderRadius: 8, flexShrink: 0 }}>
                  플랜 보기 →
                </span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {PREMIUM_FEATURES.map((f) => {
                  ...
                  return (
                    <div key={f.id}
                      onClick={() => { if (locked) router.push("/dashboard/pricing"); else router.push(f.href); }}
                      style={{ borderRadius: 14, cursor: "pointer", overflow: "hidden", border: locked ? "1px solid #ebe9e3" : "1.5px solid " + f.badgeColor + "30", background: locked ? "#faf9f6" : isPro ? "linear-gradient(135deg,rgba(201,146,10,0.07),rgba(232,150,10,0.02))" : "linear-gradient(135deg,rgba(26,39,68,0.05),rgba(15,165,115,0.03))" }}
                    ...
                    >
                      <div style={{ padding: "12px 12px 0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
                          {f.tag && <span style={{ fontSize: 9, fontWeight: 800, color: f.tagColor, background: f.tagColor + "18", padding: "2px 7px", borderRadius: 4 }}>{f.tag}</span>}
                          <span style={{ fontSize: 9, fontWeight: 800, color: locked ? "#b0b0c0" : f.badgeColor, background: locked ? "#eeeef0" : f.badgeColor + "18", padding: "2px 7px", borderRadius: 5 }}>{f.badge}</span>
                        </div>
                      </div>
                      <div style={{ padding: "8px 12px 10px" }}>
                        <p style={{ fontSize: 12, fontWeight: 800, color: locked ? "#b0b0c0" : "#1a2744", marginBottom: 4, lineHeight: 1.3 }}>{f.title}</p>
                        <p style={{ fontSize: 10, color: locked ? "#c8c8d0" : "#8a8a9a", lineHeight: 1.5 }}>{f.desc}</p>
                      </div>
                      <div style={{ padding: "8px 12px 11px", borderTop: "1px solid " + (locked ? "#f0efe9" : f.badgeColor + "15") }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: locked ? "#c0c0cc" : f.badgeColor }}>{locked ? "업그레이드 필요 →" : "바로 사용하기 →"}</span>
                      </div>
                    </div>
                  );
                })}
              ...
              {planLevel < 3 && (
                <div onClick={() => router.push("/dashboard/pricing")} style={{ marginTop: 14, borderRadius: 14, padding: "16px 18px", cursor: "pointer", background: "linear-gradient(135deg,#1a2744,#2d4270)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginTop: 14 }}>
              ...
              >
                ...
                <p style={{ fontSize: 12, fontWeight: 800, color: "#1a2744", marginBottom: 3 }}>임대인 커뮤니티</p>
                <p style={{ fontSize: 10, color: "#8a8a9a", lineHeight: 1.5 }}>팁·정보·질문 공유</p>
                <div style={{ marginTop: 8, borderTop: "1px solid #0fa57320", paddingTop: 8 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#0fa573" }}>바로가기 →</span>
                </div>
              </div>
              <div onClick={() => router.push("/dashboard/premium/global-reports")} style={{ borderRadius: 14, padding: "16px", cursor: "pointer", background: "linear-gradient(135deg,rgba(26,39,68,0.05),rgba(91,79,207,0.03))", border: "1.5px solid rgba(26,39,68,0.15)" }}
                onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; }}
                onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; }}
              >
                <div style={{ fontSize: 24, marginBottom: 6 }}>🌐</div>
                <p style={{ fontSize: 12, fontWeight: 800, color: "#1a2744", marginBottom: 3 }}>글로벌 리포트</p>
                <p style={{ fontSize: 10, color: "#8a8a9a", lineHeight: 1.5 }}>JLL·CBRE 공식 리포트</p>
                <div style={{ marginTop: 8, borderTop: "1px solid rgba(26,39,68,0.08)", paddingTop: 8 }}>
