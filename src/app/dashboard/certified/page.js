  ...
  const save = async () => {
    ...
    if (editTarget) {
      ...
      const { data, error } = await supabase
        ...
        .select()
        .single();
      if (error) { toast("수정 오류: " + error.message, "error"); setSaving(false); return; }
      setHistory((prev) => prev.map((h) => h.id === data.id ? data : h));
      toast("수정되었습니다");
    } else {
      // 신규
      if (!selectedTenant) { toast("세입자를 선택하세요", "error"); setSaving(false); return; }
      ...
      if (error) { toast("저장 오류: " + error.message, "error"); setSaving(false); return; }
...
<meta charset="utf-8"><title>내용증명</title>
<style>
...
<div class="meta">
  <div class="meta-box"><div class="meta-label">수신인 (세입자)</div><div class="meta-value">${h.tenant_name}</div></div>
  <div class="meta-box"><div class="meta-label">발송 사유</div><div class="meta-value">${h.reason}</div><div class="badge">${h.reason}</div></div>
</div>
...
<div class="footer">
  ...
  <div class="stamp-area"><div class="stamp"><div class="stamp-label">발신인 (임대인)</div><div class="stamp-box"></div></div></div>
</div>
<script>window.onload=()=>{window.print()}<\/script>
  ...
  const btnStyle = (color, outline) => ({
    fontSize: 11, padding: "4px 10px", borderRadius: 7, cursor: "pointer", fontWeight: 600,
    ...
    <div className="page-in page-padding" style={{ maxWidth: 900 }}>
      ...
      {loading ? (
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
          {history.map((h) => (
            <div key={h.id} className="hover-lift" style={{ background:"#ffffff", border:"1px solid #ebe9e3", borderRadius:14, padding:"16px 20px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                ...
                <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                  ...
                  <button onClick={() => setShowDetail(h)} style={btnStyle(C.muted, true)}>보기</button>
                  <button onClick={() => openEdit(h)} style={btnStyle(C.indigo, false)}>✏️ 수정</button>
                  <button onClick={() => downloadPDF(h)} style={btnStyle(C.emerald, false)}>📄 PDF</button>
                  <button onClick={() => setDeleteTarget(h)} style={btnStyle(C.rose, false)}>🗑 삭제</button>
                </div>
              </div>
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); setEditTarget(null); }}>
        <h2 style={{ fontSize:19, fontWeight:800, color:"#1a2744", marginBottom:16 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div>
            ...
            {editTarget ? (
              <div style={{ padding:"11px 14px", fontSize:13, color:"#1a2744", background:"#f8f7f4", border:"1px solid #ebe9e3", borderRadius:10 }}>
                {editTarget.tenant_name}
              </div>
            ) : (
              <select value={selectedTenant} onChange={(e) => setSelectedTenant(e.target.value)}
                style={{ width:"100%", padding:"11px 14px", fontSize:13, color:"#1a2744", background:"#f8f7f4", border:"1px solid #ebe9e3", borderRadius:10, outline:"none" }}>
          ...
          <div>
            ...
            <textarea value={content} onChange={(e) => setContent(e.target.value)}
              placeholder="내용증명 본문을 입력하세요..." rows={6}
          ...
          {!editTarget && (
            <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:10, padding:"10px 14px" }}>
              <p style={{ fontSize:12, color:"#166534" }}>✅ 저장 후 PDF로 출력하여 직접 우편 발송하시면 됩니다.</p>
            </div>
          )}
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            <button onClick={save} disabled={saving}
              style={{ flex:2, padding:"12px", borderRadius:11, background:`linear-gradient(135deg,${editTarget?C.indigo:C.rose},${editTarget?"#2d4270":"#dc2626"})`, border:"none", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer", opacity:saving?0.7:1 }}>
              {saving ? "처리중..." : editTarget ? "수정 완료" : "저장하기"}
            </button>
          </div>
        </div>
      </Modal>
        ...
        <Modal open={!!showDetail} onClose={() => setShowDetail(null)}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={() => downloadPDF(showDetail)}
                style={{ padding:"7px 14px", borderRadius:9, background:C.indigo, border:"none", color:"#fff", fontWeight:700, fontSize:12, cursor:"pointer" }}>📄 PDF</button>
            </div>
          </div>
        ...
        <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)}>
          <div style={{ textAlign:"center", padding:"8px 0" }}>
            ...
            <p style={{ fontSize:13, color:C.muted, marginBottom:6 }}>
              <strong style={{ color:"#1a2744" }}>{deleteTarget.tenant_name}</strong> — {deleteTarget.reason}
            </p>
            <p style={{ fontSize:12, color:C.rose, marginBottom:24 }}>삭제 후 복구할 수 없습니다.</p>
            <div style={{ display:"flex", gap:10 }}>
