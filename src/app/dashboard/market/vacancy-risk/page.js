...
// 한국부동산원 공실률 통계 (2024년 4분기 기준, 중소형 기준)
const KAB_VACANCY = {
    ...
    try {
      ...
      const monthlyData = await Promise.all(
        months.map(async ({ ym, label }) => {
          ...
          const items = d.items || [];
          const jeonse = items.filter(i => parseInt(i.monthlyRent || "0") === 0).length;
          const wolse = items.filter(i => parseInt(i.monthlyRent || "0") > 0).length;
          return { label, ym, total: items.length, jeonse, wolse };
        })
      );
      ...
      // 전세 비중 (전세 비중 낮을수록 공실 위험 상대적으로 높음)
      ...
      // 공실 위험 지수: 한국부동산원 공실률 + 거래량 추세 보정
      const baseScore = Math.min(100, Math.round(kabVacancy.rate * 6)); // 공실률 → 점수
      const trendAdj = isDecreasing ? 10 : parseFloat(trendChange) > 5 ? -5 : 0;
      const riskScore = Math.max(0, Math.min(100, baseScore + trendAdj));

      // 위험 등급 결정
      let riskLabel = riskScore >= 70 ? "위험" : riskScore >= 50 ? "주의" : riskScore >= 30 ? "보통" : "안전";
      ...
      const vacancyTrend = TREND_LABELS.map((label, i) => ({
        label,
        rate: trendBase[i] || kabVacancy.rate,
      }));
      ...
      setData({
        ...
        recent3avg: Math.round(recent3avg), prev3avg: Math.round(prev3avg),
        trendChange, jeonseRatio, kabVacancy,
        vacancyTrend, seasonData, maxSeason, totalTx,
      });
    } catch (e) {
      console.error(e);
    }
      ...
      {data && (<>
        ...
        <div style={{ background: "rgba(26,39,68,0.04)", border: "1px solid rgba(26,39,68,0.12)", borderRadius: 12, padding: "12px 16px", marginBottom: 20, display: "flex", gap: 10 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: "var(--text-muted)", lineHeight: 1.7 }}>
              ...
              거래량 추이: <strong>국토교통부 실거래가 공개시스템</strong> — {region} 아파트 임대 실거래 12개월 ({data.totalTx}건)
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 16, marginBottom: 20 }}>
          <div style={{ background: data.riskStyle.bg, border: `2px solid ${data.riskStyle.color}40`, borderRadius: 16, padding: 24, textAlign: "center" }}>
            ...
            <div style={{ marginTop: 12, padding: "6px 16px", borderRadius: 20, background: data.riskStyle.color, color: "#fff", fontSize: 13, fontWeight: 800, display: "inline-block" }}>
              {data.riskLabel}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[
              ...
              { label: "최근 3개월 월평균 거래량", value: `${data.recent3avg}건`, sub: `이전 3개월 ${data.prev3avg}건` },
          ...
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 16px 10px" }}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={data.monthlyData}>
                ...
                <Tooltip contentStyle={{ borderRadius: 10, border: "1px solid var(--border)", background: "var(--surface)" }}
                  formatter={(v, n) => [`${v}건`, n === "total" ? "전체" : n === "jeonse" ? "전세" : "월세"]} />
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 16px 10px" }}>
          ...
          <p style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 14 }}>
            거래량 많은 달 = 수요 피크 → 이 시기에 계약 만기 맞추면 공실 위험 최소화 가능
          </p>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={data.seasonData}>
              ...
              <Tooltip contentStyle={{ borderRadius: 10, border: "1px solid var(--border)", background: "var(--surface)" }}
                formatter={(v) => [`${v}건`]} />
              <Bar dataKey="avg" radius={[4, 4, 0, 0]} opacity={0.9}>
                {data.seasonData.map((d, i) => (
                  <rect key={i} fill={d.avg === data.maxSeason ? "#1a2744" : "#c7d2e8"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          <p style={{ fontSize: 11, color: "var(--text-faint)", textAlign: "center", marginTop: 8 }}>
            출처: 국토교통부 실거래가 공개시스템 · 최근 12개월
          </p>
        </div>
      </>)}
