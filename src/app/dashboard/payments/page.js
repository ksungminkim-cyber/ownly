...
// ── 입금문자 파싱 고도화 ─────────────────────────────────────────
// 카카오뱅크, 토스, 국민, 신한, 하나, 우리, 기업, NH 패턴 지원
function parseBankSms(text) {
  ...
  // 패턴1: "1,100만원" or "120만원"
  const manMatch = text.match(/([0-9,]+)\s*만원/);
  if (manMatch) amt = parseInt(manMatch[1].replace(/,/g, ""), 10);

  // 패턴2: "1,100,000원" (원 단위 → 만원 변환)
  if (!amt) {
    const wonMatch = text.match(/([0-9,]{4,})\s*원/);
    if (wonMatch) {
      const rawWon = parseInt(wonMatch[1].replace(/,/g, ""), 10);
      amt = Math.round(rawWon / 10000);
    }
  }

  // 패턴3: 카카오뱅크 "입금 1,100,000" (단위 없음)
  if (!amt) {
    const kakaoMatch = text.match(/입금\s*([0-9,]+)/);
    if (kakaoMatch) {
      const raw = parseInt(kakaoMatch[1].replace(/,/g, ""), 10);
      amt = raw > 9999 ? Math.round(raw / 10000) : raw;
    }
  }
  ...
  // 은행명 추출
  const banks = ["카카오뱅크", "토스뱅크", "케이뱅크", "국민", "신한", "하나", "우리", "기업", "농협", "NH", "IBK", "KB", "SC"];
  const bank = banks.find(b => text.includes(b)) || null;

  return { amt, paidDate, bank };
}

// 이메일 알림 — 대시보드 방문 시 하루 한 번 트리거
export function triggerDailyNotify(user, tenants, payments) {
  if (!user?.id || !user?.email || !tenants?.length) return;
  const today = new Date().toISOString().slice(0, 10);
  const key = `ownly_notified_${today}_${user.id}`;
  ...
  // 미납 체크
  const hasUnpaid = tenants.some(t => {
    const p = (payments || []).find(x => x.tid === t.id && x.month === month && (x.year || year) === year);
    return !p || p.status !== "paid";
  });

  // 만료 임박 체크 (90일 이내)
  const hasExpiring = tenants.some(t => {
  ...
  const types = [];
  if (hasUnpaid)    types.push("unpaid");
  if (hasExpiring)  types.push("expiring");
  if (isFirstOfMonth) types.push("checklist");
...
export default function PaymentsPage() {
  ...
  const displayRows = viewMode === "maintenance" ? rows.filter((r) => hasMaintenance(r.t)) : rows;
    ...
    <div className="page-in page-padding" style={{ maxWidth: 1020 }}>
      ...
      <div className="dash-grid-4" style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 12, marginBottom: 18 }}>
        {[
          ...
          { l: "관리비 청구", v: totalMaintExp.toLocaleString() + "만원", c: C.muted, hide: maintTenants.length === 0 },
          { l: "관리비 수금", v: totalMaintColl.toLocaleString() + "만원", c: C.emerald, hide: maintTenants.length === 0 },
      ...
      {maintTenants.length > 0 && (
        <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
          {[{ v: "all", l: "전체" }, { v: "rent", l: "월세만" }, { v: "maintenance", l: "관리비 해당만" }].map(({ v, l }) => (
            <button key={v} onClick={() => setViewMode(v)} style={{ padding: "5px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer", border: `1px solid ${viewMode === v ? "#1a2744" : "#ebe9e3"}`, background: viewMode === v ? "#1a2744" : "transparent", color: viewMode === v ? "#fff" : "#8a8a9a" }}>{l}</button>
          ))}
        </div>
      )}
                  ...
                  return (
                    <tr key={t.id} className="trow" style={{ borderTop: "1px solid #ebe9e3", background: st === "unpaid" ? C.rose + "04" : "transparent" }}>
                      <td style={{ padding: "11px 14px" }}>
                        <div style={{ display: "flex", gap: 9, alignItems: "center" }}>
                          ...
                          <div>
                            ...
                            {t.phone && (
                              <a href={`tel:${t.phone}`} style={{ fontSize: 10, color: "#0fa573", textDecoration: "none", fontWeight: 600 }}>
                                📞 {t.phone}
                              </a>
                            )}
                          </div>
                      ...
                      <td style={{ padding: "11px 14px", fontSize: 13, fontWeight: 700, color: "#1a2744", whiteSpace: "nowrap" }}>
                        {Number(t.rent).toLocaleString()}만원
                      ...
                      {maintTenants.length > 0 && (
                        <>
                          <td style={{ padding: "11px 14px", fontSize: 13, fontWeight: 700, color: showMaint ? "#1a2744" : "#d1d5db", whiteSpace: "nowrap" }}>
                            {showMaint ? `${Number(t.maintenance).toLocaleString()}만원` : "—"}
                          </td>
                          <td style={{ padding: "11px 14px" }}>
                            {showMaint ? (
                              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                ...
                <div style={{ display: "flex", gap: 9 }}>
                  ...
                  <button onClick={() => markPaid(payModal)} disabled={saving} className="btn-primary" style={{ flex: 2, padding: "11px", borderRadius: 10, background: `linear-gradient(135deg,${C.emerald},#059669)`, border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                    {saving ? "처리 중..." : "납부 확인"}
                  </button>
                </div>
              </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                ...
                <div style={{ display: "flex", gap: 9 }}>
                  ...
                  <button onClick={() => markMaintPaid(maintModal)} disabled={saving} className="btn-primary" style={{ flex: 2, padding: "11px", borderRadius: 10, background: `linear-gradient(135deg,${C.amber},#d97706)`, border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                    {saving ? "처리 중..." : "납부 확인"}
                  </button>
                </div>
              </div>
      ...
      <Modal open={smsModal} onClose={() => { setSmsModal(false); setSmsText(""); setSmsParsed(null); setSmsTid(null); }} width={440}>
        ...
        <p style={{ fontSize: 12, color: "#8a8a9a", marginBottom: 16 }}>
          카카오뱅크·토스·국민·신한·하나·우리 입금 알림을 붙여넣으면<br/>금액과 날짜를 자동으로 인식합니다
        </p>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea value={smsText} onChange={(e) => handleSmsChange(e.target.value)}
            placeholder={"예시:\n[카카오뱅크] 03/05 입금 1,100,000원 잔액 2,340,000원\n[국민은행] 120만원 입금 03-05 홍길동\n[토스] 홍길동님이 1,100,000원을 보냈어요"}
            rows={5} style={{ width: "100%", padding: "11px 13px", fontSize: 13, color: "#1a2744", background: "#f8f7f4", border: "1px solid #ebe9e3", borderRadius: 10, resize: "vertical", outline: "none", fontFamily: "inherit" }} />
        </div>

        {smsText && (
          <div style={{ padding: "14px 16px", borderRadius: 12, marginBottom: 14, background: smsParsed ? "rgba(15,165,115,0.07)" : "rgba(232,68,90,0.07)", border: `1px solid ${smsParsed ? "rgba(15,165,115,0.25)" : "rgba(232,68,90,0.25)"}` }}>
            {smsParsed ? (
              <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
                <div>
                  <p style={{ fontSize: 10, color: "#8a8a9a", fontWeight: 700, marginBottom: 3 }}>인식된 금액</p>
                  <p style={{ fontSize: 18, fontWeight: 900, color: "#0fa573" }}>{Number(smsParsed.amt).toLocaleString()}만원</p>
                </div>
                <div>
                ...
                {smsParsed.bank && (
                  <div>
                    <p style={{ fontSize: 10, color: "#8a8a9a", fontWeight: 700, marginBottom: 3 }}>은행</p>
                    <p style={{ fontSize: 13, fontWeight: 700, color: "#5b4fcf" }}>{smsParsed.bank}</p>
                  </div>
                )}
              </div>
            ) : (
              <div>
                <p style={{ fontSize: 12, color: "#e8445a", fontWeight: 700, marginBottom: 4 }}>❌ 금액을 인식하지 못했습니다</p>
                <p style={{ fontSize: 11, color: "#8a8a9a" }}>금액이 "120만원" 또는 "1,200,000원" 형식으로 포함되어 있는지 확인하세요</p>
              </div>
            )}
          </div>
        )}
      ...
      {/* 미납 법적 대응 배너 */}
      {tenants.some((t) => { const p = payments.find((x) => x.tid === t.id && x.month === month); return !p || p.status === "unpaid"; }) && (
        <div style={{ marginTop: 20, background: "linear-gradient(135deg,rgba(232,68,90,0.05),rgba(232,68,90,0.02))", border: "1px solid rgba(232,68,90,0.2)", borderRadius: 16, padding: "18px 22px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => router.push("/dashboard/certified")} style={{ padding: "9px 16px", borderRadius: 10, background: "transparent", border: "1px solid rgba(232,68,90,0.4)", color: "#e8445a", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>내용증명 작성</button>
              <button onClick={() => alert("🚧 법무사 연결 서비스 준비 중입니다.\n빠른 시일 내에 오픈할게요!")} style={{ padding: "9px 16px", borderRadius: 10, background: "#e8445a", border: "none", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>법무사 연결 →</button>
            </div>
          </div>
        </div>
      )}
