"use client"; import { useState, useMemo, useEffect } from "react"; import { useRouter } from "next/navigation"; import { Badge, SectionLabel, SearchBox, EmptyState, ConfirmDialog, Modal, AuthInput, SortButton, toast, InlineLoader } from "../../../components/shared"; import { C, STATUS_MAP, COLORS, daysLeft, groupByBuilding } from "../../../lib/constants"; import { useApp } from "../../../context/AppContext"; import AddressInput from "../../../components/AddressInput"; function calcRentPerPyeong(rent, area_pyeong) { if (!rent || !area_pyeong || area_pyeong <= 0) return null; return Math.round((Number(rent) / Number(area_pyeong)) * 10) / 10; } function formatPayDay(pay_day) { if (!pay_day) return ""; if (Number(pay_day) === 99) return "말일"; return `매월 ${pay_day}일`; }
                        ...
                        <div style={{ flex: 1, background: h.isCurrent ? typeColor + "08" : "#fafaf8", border: `1px solid ${h.isCurrent ? typeColor + "30" : "#ebe9e3"}`, borderRadius: 12, padding: "12px 14px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                              ...
                              {h.isCurrent && (
                                <span style={{ fontSize: 10, fontWeight: 700, color: typeColor, background: typeColor + "18", padding: "2px 7px", borderRadius: 10 }}>현재</span>
                              )}
                              <span style={{ fontSize: 10, fontWeight: 600, color: "#8a8a9a", background: "#f0efe9", padding: "2px 6px", borderRadius: 5 }}>{h.source}</span>
                            </div>
                          ...
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(100px,1fr))", gap: 8 }}>
                            {[
                              ...
                              <div key={k.l} style={{ background: "#fff", borderRadius: 8, padding: "6px 10px" }}>
                                <p style={{ fontSize: 10, color: "#8a8a9a", fontWeight: 700, marginBottom: 2 }}>{k.l}</p>
...
export default function PropertiesPage() { const router = useRouter(); const { tenants, buildings, addTenant, updateTenant, deleteTenant, contracts, addContract, updateContract, loading, canUse, getPlanLimit, user } = useApp(); const [activeTab, setActiveTab] = useState("properties"); const [filter, setFilter] = useState("전체"); const [search, setSearch] = useState(""); const [sort, setSort] = useState({ key: "rent", dir: "desc" }); const [viewMode, setViewMode] = useState("flat"); const [expandedBuildings, setExpandedBuildings] = useState({}); const [showModal, setShowModal] = useState(false); const [editTarget, setEditTarget] = useState(null); const [deleteTarget, setDeleteTarget] = useState(null); const [saving, setSaving] = useState(false); const [form, setForm] = useState({ pType: "주거", sub: "아파트", addr: "", rent: "", dep: "", name: "", phone: "", start: "", end: "", maintenance: "", pay_day: "5", area_pyeong: "", isVacant: false, industry: "", premium: "", rentFree: "", vatIncluded: false, building_id: null });
  ...
  {activeTab === "contracts" && ( <div> <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}> <button onClick={() => { setContractEdit(null); setContractForm({ tenant_id: "", start_date: "", end_date: "", rent: "", deposit: "", special_terms: "" }); setShowContractModal(true); }} style={{ padding: "9px 18px", borderRadius: 10, background: `linear-gradient(135deg,#1a2744,#5b4fcf)`, border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>+ 계약서 작성</button> </div> {!contracts?.length ? ( <EmptyState icon="📝" title="등록된 계약서가 없습니다" desc="계약서 작성 버튼으로 첫 계약서를 등록하세요" /> ) : ( <div style={{ background: "#fff", border: "1px solid #ebe9e3", borderRadius: 16, overflow: "hidden" }}> {contracts.map((c, i) => { const tenant = tenants.find(t => t.id === c.tenant_id); const dl = c.end_date ? Math.ceil((new Date(c.end_date) - new Date()) / 86400000) : null; return ( <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", borderBottom: i < contracts.length - 1 ? "1px solid #f0efe9" : "none" }}> <div> <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}> <span style={{ fontSize: 14, fontWeight: 700, color: "#1a2744" }}>{tenant?.name || c.tenant_name || "알수없음"}</span> {dl !== null && <span style={{ fontSize: 10, fontWeight: 700, color: dl <= 90 ? "#e8445a" : "#8a8a9a", background: dl <= 90 ? "rgba(232,68,90,0.1)" : "#f0efe9", padding: "2px 8px", borderRadius: 20 }}>만료 D-{dl}</span>} </div> <p style={{ fontSize: 12, color: "#8a8a9a" }}>{c.start_date || "—"} ~ {c.end_date || "—"} · 월세 {Number(c.rent || tenant?.rent || 0).toLocaleString()}만원 · 보증금 {Number(c.deposit || tenant?.dep || 0).toLocaleString()}만원</p> {c.special_terms && <p style={{ fontSize: 11, color: "#8a8a9a", marginTop: 3 }}>📌 {c.special_terms.slice(0, 60)}{c.special_terms.length > 60 ? "..." : ""}</p>} </div> <div style={{ display: "flex", gap: 6, flexShrink: 0 }}> <button onClick={() => { try { const landlord = { name: user?.user_metadata?.landlord_name || user?.user_metadata?.nickname || "", phone: user?.user_metadata?.phone || "", addr: user?.user_metadata?.landlord_addr || "", reg: user?.user_metadata?.business_no || "" }; localStorage.setItem("ownly_contract_print", JSON.stringify({ contract: c, tenant, landlord })); window.open("/dashboard/contracts/export", "_blank"); } catch (e) { toast("PDF 생성 중 오류: " + e.message, "error"); } }} style={{ padding: "5px 12px", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer", border: "1px solid rgba(91,79,207,0.3)", background: "rgba(91,79,207,0.06)", color: "#5b4fcf" }}>📄 PDF</button> <button onClick={() => { setContractEdit(c); setContractForm({ tenant_id: c.tenant_id || "", start_date: c.start_date || "", end_date: c.end_date || "", rent: c.rent || "", deposit: c.deposit || "", special_terms: c.special_terms || "" }); setShowContractModal(true); }} style={{ padding: "5px 12px", borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: "pointer", border: "1px solid #ebe9e3", background: "transparent", color: "#1a2744" }}>수정</button> </div> </div> ); })} </div> )} </div> )}
