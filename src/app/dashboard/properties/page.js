...
// 평당 월세 계산 (만원/평)
function calcRentPerPyeong(rent, area_pyeong) {
  if (!rent || !area_pyeong || area_pyeong <= 0) return null;
  return Math.round((Number(rent) / Number(area_pyeong)) * 10) / 10;
}
...
export default function PropertiesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      else if (sort.key === "rentPerPyeong") {
        va = calcRentPerPyeong(a.rent, a.area_pyeong) ?? -1;
        vb = calcRentPerPyeong(b.rent, b.area_pyeong) ?? -1;
      }
  ...
  const openEdit = (t) => {
    ...
    setForm({
      ...
      area_pyeong: t.area_pyeong ? String(t.area_pyeong) : "",
    });
    setShowModal(true);
  };
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                    <div style={{ display: "flex", gap: 13, alignItems: "center", flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        <div style={{ display: "flex", gap: 7, marginBottom: 5, flexWrap: "wrap" }}>
                          ...
                          {/* ✅ 평형 배지 */}
                          {t.area_pyeong && (
                            <span style={{ fontSize: 10, fontWeight: 700, color: "#64748b", background: "#f1f5f9", padding: "2px 7px", borderRadius: 5 }}>
                              {t.area_pyeong}평
                            </span>
                          )}
                        </div>
                        ...
                        <p style={{ fontSize: 11, color: "#8a8a9a", marginTop: 2 }}>👤 {t.name} · {t.phone}{t.pay_day ? ` · 매월 ${t.pay_day}일 납부` : ""}</p>
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                      <div style={{ textAlign: "right" }}>
                        ...
                        {/* ✅ 평당 월세 */}
                        {rentPerPyeong !== null ? (
                          <p style={{ fontSize: 11, color: "#5b4fcf", fontWeight: 700, marginTop: 3 }}>평당 {rentPerPyeong}만원</p>
                        ) : (
                          <p style={{ fontSize: 10, color: "#c4c2bc", marginTop: 3 }}>평형 미입력</p>
                        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {[{ type: "주거", icon: "🏠", defaultSub: "아파트" }, { type: "상가", icon: "🏪", defaultSub: "1층 상가" }, { type: "토지", icon: "🌱", defaultSub: "전·답" }].map(({ type, icon, defaultSub }) => (
                <button key={type} onClick={() => setForm((f) => ({ ...f, pType: type, sub: defaultSub }))} style={{ flex: 1, padding: "11px", borderRadius: 10, border: `2px solid ${form.pType === type ? (type === "상가" ? C.amber : type === "토지" ? "#0d9488" : C.indigo) : "#ebe9e3"}`, background: form.pType === type ? (type === "상가" ? C.amber : type === "토지" ? "#0d9488" : C.indigo) + "18" : "transparent", color: form.pType === type ? (type === "상가" ? C.amber : type === "토지" ? "#0d9488" : C.indigo) : C.muted, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                  {icon} {type}
                </button>
              ))}
            </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {(form.pType === "주거" ? ["아파트", "빌라", "오피스텔", "단독주택", "다세대"] : form.pType === "상가" ? ["1층 상가", "2층 이상", "지하", "사무실", "창고·물류"] : ["전·답", "임야", "대지", "잡종지", "기타 토지"]).map((s) => (
                  <button key={s} onClick={() => setForm((f) => ({ ...f, sub: s }))} style={{ padding: "6px 12px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", border: "none", background: form.sub === s ? (form.pType === "상가" ? C.amber : form.pType === "토지" ? "#0d9488" : C.indigo) : "#f0efe9", color: form.sub === s ? "#fff" : "#8a8a9a", transition: "all .15s" }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            {/* ✅ 평형 입력 + 평당 월세 실시간 미리보기 */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <AuthInput
                label="전용면적 (평)"
                placeholder="예: 15 (약 49㎡)"
                type="number"
                value={form.area_pyeong}
                onChange={(e) => setForm((f) => ({ ...f, area_pyeong: e.target.value }))}
                icon="📐"
              />
              <div style={{ background: "rgba(91,79,207,0.05)", border: "1px solid rgba(91,79,207,0.2)", borderRadius: 10, padding: "10px 13px", display: "flex", alignItems: "center" }}>
                <div>
                  ...
                  <p style={{ fontSize: 16, fontWeight: 800, color: "#1a2744" }}>
                    {calcRentPerPyeong(form.rent, form.area_pyeong)
                      ? `${calcRentPerPyeong(form.rent, form.area_pyeong)}만원/평`
                      : "—"}
                  </p>
            ...
            <div style={{ background: "rgba(26,39,68,0.03)", border: "1px solid rgba(26,39,68,0.1)", borderRadius: 12, padding: "14px 16px" }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
                {[1, 5, 10, 15, 20, 25].map(d => (
                  <button key={d} onClick={() => setForm(f => ({ ...f, pay_day: String(d) }))} style={{ padding: "6px 14px", borderRadius: 20, fontSize: 13, fontWeight: 700, cursor: "pointer", border: `1.5px solid ${form.pay_day === String(d) ? C.indigo : "#ebe9e3"}`, background: form.pay_day === String(d) ? C.indigo : "transparent", color: form.pay_day === String(d) ? "#fff" : "#8a8a9a", transition: "all .15s" }}>
                    {d}일
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
              ...
              <button onClick={saveTenant} disabled={saving} className="btn-primary" style={{ flex: 2, padding: "12px", borderRadius: 11, background: `linear-gradient(135deg,${C.indigo},${C.purple})`, border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", opacity: saving ? 0.7 : 1 }}>
