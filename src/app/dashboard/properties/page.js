...
export default function PropertiesPage() {
  ...
  const [form, setForm] = useState({ pType: "주거", sub: "아파트", addr: "", rent: "", dep: "", name: "", phone: "", start: "", end: "", maintenance: "", pay_day: "5", area_pyeong: "", isVacant: false, unitLabel: "" });
  ...
  const groupedView = useMemo(() => {
    ...
    return order.map(addr => ({ addr, items: map[addr] }));
  }, [filtered]);
  ...
  const openEdit = (t) => {
    setEditTarget(t);
    setForm({ pType: t.pType, sub: t.sub, addr: t.addr, rent: String(t.rent), dep: String(t.dep), name: t.status === "공실" ? "" : t.name, phone: t.phone || "", start: t.start_date || "", end: t.end_date || "", maintenance: String(t.maintenance || ""), pay_day: String(t.pay_day || "5"), area_pyeong: t.area_pyeong ? String(t.area_pyeong) : "", isVacant: t.status === "공실", unitLabel: t.biz || "" });
    setShowModal(true);
  };

  const saveTenant = async () => {
    if (!form.addr) { toast("주소를 입력하세요", "error"); return; }
    if (!form.isVacant && (!form.rent || !form.name)) { toast("필수 항목을 입력하세요", "error"); return; }
    const payDayRaw = form.pay_day === "99" ? 99 : parseInt(form.pay_day || "5");
    if (!form.isVacant && (isNaN(payDayRaw) || (payDayRaw !== 99 && (payDayRaw < 1 || payDayRaw > 31)))) { toast("납부일은 1~31일 또는 말일로 입력하세요", "error"); return; }
    setSaving(true);
    try {
      const color = COLORS[Math.floor(Math.random() * COLORS.length)];
      const payload = { name: form.isVacant ? "공실" : form.name, phone: form.isVacant ? "" : (form.phone || ""), pType: form.pType, sub: form.sub, addr: form.addr, dep: Number(form.dep || 0), rent: Number(form.rent || 0), start_date: form.start || null, end_date: form.end || "2026-12-31", status: form.isVacant ? "공실" : "정상", color, intent: "미확인", maintenance: Number(form.maintenance || 0), pay_day: form.isVacant ? 0 : payDayRaw, biz: form.unitLabel || null, contacts: [], area_pyeong: form.area_pyeong ? Number(form.area_pyeong) : null };
      if (editTarget) { await updateTenant(editTarget.id, payload); toast("물건 정보가 수정되었습니다"); }
      else { await addTenant(payload); toast(form.isVacant ? "공실이 등록되었습니다" : "새 물건이 등록되었습니다"); }
  ...
  const handleDelete = async () => {
    ...
    try { await deleteTenant(deleteTarget.id); toast(deleteTarget.name + "님 물건이 삭제되었습니다"); }
  ...
  // ✅ 카드 렌더링 — 단일/그룹 모두 동일한 카드 스타일, 그룹 내 카드만 왼쪽 라인으로 구분
  const renderCard = (t, groupSize, groupIndex) => {
    ...
    // 그룹 내 카드: 왼쪽 인디고 라인, 첫/마지막 border-radius 조정으로 연결감
    const groupStyle = inGroup ? {
      borderLeft: `3px solid ${vacant ? "#f59e0b" : C.indigo}`,
      borderRadius: isFirst && isLast ? 14 : isFirst ? "14px 14px 4px 4px" : isLast ? "4px 4px 14px 14px" : "4px",
      marginBottom: isLast ? 0 : 2,
    } : {};

    return (
      <div key={t.id} className="hover-lift" style={{ background: vacant ? "rgba(251,191,36,0.04)" : "#ffffff", border: `1px solid ${vacant ? "#fbbf2440" : "#ebe9e3"}`, borderRadius: 14, padding: "15px 18px", ...groupStyle }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center", flex: 1, minWidth: 0 }}>
            <div style={{ width: 40, height: 40, borderRadius: 11, background: vacant ? "#fbbf2420" : (t.color || C.indigo) + "20", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>
              {vacant ? "🔑" : t.pType === "상가" ? "🏪" : t.pType === "토지" ? "🌱" : "🏠"}
            </div>
            <div style={{ minWidth: 0 }}>
              <div style={{ display: "flex", gap: 6, marginBottom: 4, flexWrap: "wrap", alignItems: "center" }}>
                {t.biz && <span style={{ fontSize: 10, fontWeight: 800, color: C.indigo, background: C.indigo + "18", padding: "2px 7px", borderRadius: 5 }}>{t.biz}</span>}
              ...
              {/* 단일 물건이면 주소 표시, 그룹 내이면 주소 생략 (이미 그룹 요약에서 보임) */}
              {!inGroup && <p style={{ fontSize: 14, fontWeight: 700, color: "#1a2744", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "100%" }}>{t.addr}</p>}
              {vacant
            ...
            <div style={{ textAlign: "right" }}>
              {vacant ? (
                ...
                <>
                  ...
                  {rentPerPyeong !== null ? <p style={{ fontSize: 11, color: "#5b4fcf", fontWeight: 700, marginTop: 2 }}>평당 {rentPerPyeong}만원</p> : <p style={{ fontSize: 10, color: "#c4c2bc", marginTop: 2 }}>평형 미입력</p>}
  ...
  // ✅ 그룹 요약 바 — 그룹일 때만 카드 위에 작은 배지로 표시
  const renderGroupSummary = (addr, items) => {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5, paddingLeft: 4 }}>
        ...
        <p style={{ fontSize: 12, fontWeight: 800, color: "#1a2744", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{addr}</p>
        <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: "#0fa573", background: "rgba(15,165,115,0.1)", padding: "2px 8px", borderRadius: 20 }}>입주 {occupiedCount}</span>
          {vacantCount > 0 && <span style={{ fontSize: 10, fontWeight: 700, color: "#d97706", background: "#fef3c7", padding: "2px 8px", borderRadius: 20 }}>공실 {vacantCount}</span>}
          <span style={{ fontSize: 10, fontWeight: 700, color: "#8a8a9a", background: "#f0efe9", padding: "2px 8px", borderRadius: 20 }}>월 {totalRent.toLocaleString()}만</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="page-in page-padding" style={{ maxWidth: 920 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
        ...
        <button onClick={() => { const limit = getPlanLimit("properties"); if (limit !== Infinity && tenants.length >= limit) { alert(`현재 플랜에서는 물건을 최대 ${limit}개까지 등록할 수 있어요.\n업그레이드하면 더 많이 등록할 수 있어요!`); window.location.href = "/dashboard/pricing"; return; } resetForm(); setEditTarget(null); setShowModal(true); }} className="btn-primary" style={{ padding: "10px 20px", borderRadius: 11, background: `linear-gradient(135deg,${C.indigo},${C.purple})`, border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
          + 물건 추가
        </button>
      </div>

      <div style={{ display: "flex", gap: 4, marginBottom: 18, borderBottom: "1px solid #ebe9e3", paddingBottom: 0 }}>
        {[{ key: "properties", label: "🏠 물건 목록", count: tenants.length }, { key: "contracts", label: "📝 계약서", count: contracts?.length || 0 }].map(tab => (
      ...
      {activeTab === "contracts" && (
        <div>
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
            <button onClick={() => { setContractEdit(null); setContractForm({ tenant_id: "", start_date: "", end_date: "", rent: "", deposit: "", special_terms: "" }); setShowContractModal(true); }} style={{ padding: "9px 18px", borderRadius: 10, background: `linear-gradient(135deg,#1a2744,#5b4fcf)`, border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              + 계약서 작성
            </button>
          </div>
          {!contracts?.length ? (
                ...
                return (
                  <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", borderBottom: i < contracts.length - 1 ? "1px solid #f0efe9" : "none" }}>
                    <div>
                      ...
                      <p style={{ fontSize: 12, color: "#8a8a9a" }}>{c.start_date || "—"} ~ {c.end_date || "—"} &nbsp;·&nbsp; 월세 {Number(c.rent || tenant?.rent || 0).toLocaleString()}만원 &nbsp;·&nbsp; 보증금 {Number(c.deposit || tenant?.dep || 0).toLocaleString()}만원</p>
                      {c.special_terms && <p style={{ fontSize: 11, color: "#8a8a9a", marginTop: 3 }}>📌 {c.special_terms.slice(0, 60)}{c.special_terms.length > 60 ? "..." : ""}</p>}
                    </div>
                    <button onClick={() => { setContractEdit(c); setContractForm({ tenant_id: c.tenant_id || "", start_date: c.start_date || "", end_date: c.end_date || "", rent: c.rent || "", deposit: c.deposit || "", special_terms: c.special_terms || "" }); setShowContractModal(true); }} style={{ padding: "5px 12px", borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: "pointer", border: "1px solid #ebe9e3", background: "transparent", color: "#1a2744" }}>수정</button>
                  </div>
                );
              })}
            </div>
      ...
      <Modal open={showContractModal} onClose={() => setShowContractModal(false)}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
          <div>
            ...
            <select value={contractForm.tenant_id} onChange={e => setContractForm(f => ({ ...f, tenant_id: e.target.value }))} style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #ebe9e3", fontSize: 13, color: "#1a2744", background: "#f8f7f4", outline: "none" }}>
              <option value="">선택하세요</option>
              {tenants.filter(t => t.status !== "공실").map(t => <option key={t.id} value={t.id}>{t.name} — {t.addr}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            <textarea value={contractForm.special_terms} onChange={e => setContractForm(f => ({ ...f, special_terms: e.target.value }))} placeholder="특약 사항을 입력하세요..." rows={3} style={{ width: "100%", padding: "11px 13px", fontSize: 13, color: "#1a2744", background: "#f8f7f4", border: "1px solid #ebe9e3", borderRadius: 10, resize: "vertical", outline: "none", fontFamily: "inherit" }} />
          </div>
      ...
      {activeTab === "properties" && (<>
        <div style={{ display: "flex", gap: 10, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 5 }}>
            ...
            <SortButton label="평당" active={sort.key === "rentPerPyeong"} dir={sort.dir} onClick={() => toggleSort("rentPerPyeong")} />
        ...
        {loading ? <InlineLoader rows={4} /> : filtered.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
            {groupedView.map(({ addr, items }) => {
              const inGroup = items.length > 1;
              return (
                <div key={addr}>
                  {inGroup && renderGroupSummary(addr, items)}
                  {items.map((t, idx) => renderCard(t, items.length, idx))}
                </div>
              );
            })}
          </div>
        ...
        <Modal open={showModal} onClose={null}>
          ...
          <div onClick={() => setForm(f => ({ ...f, isVacant: !f.isVacant }))} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, background: form.isVacant ? "#fffbeb" : "#f8f7f4", border: `1.5px solid ${form.isVacant ? "#fbbf24" : "#ebe9e3"}`, borderRadius: 10, padding: "10px 14px", cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ display: "flex", gap: 10 }}>
              {[{ type: "주거", icon: "🏠", defaultSub: "아파트" }, { type: "상가", icon: "🏪", defaultSub: "1층 상가" }, { type: "토지", icon: "🌱", defaultSub: "전·답" }].map(({ type, icon, defaultSub }) => (
                <button key={type} onClick={() => setForm((f) => ({ ...f, pType: type, sub: defaultSub }))} style={{ flex: 1, padding: "11px", borderRadius: 10, border: `2px solid ${form.pType === type ? (type === "상가" ? C.amber : type === "토지" ? "#0d9488" : C.indigo) : "#ebe9e3"}`, background: form.pType === type ? (type === "상가" ? C.amber : type === "토지" ? "#0d9488" : C.indigo) + "18" : "transparent", color: form.pType === type ? (type === "상가" ? C.amber : type === "토지" ? "#0d9488" : C.indigo) : C.muted, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>{icon} {type}</button>
              ))}
            </div>
            <div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {(form.pType === "주거" ? ["아파트", "빌라", "오피스텔", "단독주택", "다세대"] : form.pType === "상가" ? ["1층 상가", "2층 이상", "지하", "사무실", "창고·물류"] : ["전·답", "임야", "대지", "잡종지", "기타 토지"]).map((s) => (
                  <button key={s} onClick={() => setForm((f) => ({ ...f, sub: s }))} style={{ padding: "6px 12px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", border: "none", background: form.sub === s ? (form.pType === "상가" ? C.amber : form.pType === "토지" ? "#0d9488" : C.indigo) : "#f0efe9", color: form.sub === s ? "#fff" : "#8a8a9a", transition: "all .15s" }}>{s}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <AuthInput label={form.isVacant ? "희망 보증금 (만원)" : form.pType === "토지" ? "토지 가액 (만원)" : "보증금 (만원)"} placeholder="50000" value={form.dep} onChange={(e) => setForm((f) => ({ ...f, dep: e.target.value }))} icon="💵" />
              <AuthInput label={form.isVacant ? "희망 월세 (만원)" : form.pType === "토지" ? "월 임대료 (만원)" : "월세 (만원) *"} placeholder="120" value={form.rent} onChange={(e) => setForm((f) => ({ ...f, rent: e.target.value }))} icon="💰" />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <div style={{ background: "rgba(91,79,207,0.05)", border: "1px solid rgba(91,79,207,0.2)", borderRadius: 10, padding: "10px 13px", display: "flex", alignItems: "center" }}>
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#5b4fcf", marginBottom: 3 }}>평당 월세</p>
                  <p style={{ fontSize: 16, fontWeight: 800, color: "#1a2744" }}>{calcRentPerPyeong(form.rent, form.area_pyeong) ? `${calcRentPerPyeong(form.rent, form.area_pyeong)}만원/평` : "—"}</p>
                  <p style={{ fontSize: 10, color: "#8a8a9a", marginTop: 2 }}>월세 ÷ 평형</p>
                </div>
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <div style={{ background: "rgba(26,39,68,0.03)", border: "1px solid rgba(26,39,68,0.1)", borderRadius: 12, padding: "14px 16px" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
                  {[1, 5, 10, 15, 20, 25].map(d => (
                    <button key={d} onClick={() => setForm(f => ({ ...f, pay_day: String(d) }))} style={{ padding: "6px 14px", borderRadius: 20, fontSize: 13, fontWeight: 700, cursor: "pointer", border: `1.5px solid ${form.pay_day === String(d) ? C.indigo : "#ebe9e3"}`, background: form.pay_day === String(d) ? C.indigo : "transparent", color: form.pay_day === String(d) ? "#fff" : "#8a8a9a", transition: "all .15s" }}>{d}일</button>
                  ))}
                  <button onClick={() => setForm(f => ({ ...f, pay_day: "99" }))} style={{ padding: "6px 14px", borderRadius: 20, fontSize: 13, fontWeight: 700, cursor: "pointer", border: `1.5px solid ${form.pay_day === "99" ? C.indigo : "#ebe9e3"}`, background: form.pay_day === "99" ? C.indigo : "transparent", color: form.pay_day === "99" ? "#fff" : "#8a8a9a", transition: "all .15s" }}>말일</button>
                </div>
                {form.pay_day !== "99" && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 12, color: "#8a8a9a" }}>직접 입력:</span>
                    <input type="number" min={1} max={31} value={form.pay_day} onChange={e => setForm(f => ({ ...f, pay_day: e.target.value }))} style={{ width: 70, padding: "6px 10px", borderRadius: 8, border: "1.5px solid #ebe9e3", fontSize: 13, fontWeight: 700, color: "#1a2744", textAlign: "center", outline: "none" }} />
            ...
            {(form.pType === "상가" || form.sub === "오피스텔") && !form.isVacant && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                ...
                <div style={{ background: "rgba(15,165,115,0.06)", border: "1px solid rgba(15,165,115,0.2)", borderRadius: 10, padding: "10px 13px", display: "flex", alignItems: "center" }}>
                  <div>
                    ...
                    <p style={{ fontSize: 13, fontWeight: 800, color: "#1a2744" }}>{form.rent && form.maintenance ? `월 ${(Number(form.rent) + Number(form.maintenance)).toLocaleString()}만원` : form.rent ? `월 ${Number(form.rent).toLocaleString()}만원` : "—"}</p>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
              ...
              <button onClick={saveTenant} disabled={saving} className="btn-primary" style={{ flex: 2, padding: "12px", borderRadius: 11, background: form.isVacant ? "linear-gradient(135deg,#d97706,#f59e0b)" : `linear-gradient(135deg,${C.indigo},${C.purple})`, border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", opacity: saving ? 0.7 : 1 }}>
                {saving ? "저장 중..." : editTarget ? "저장하기" : form.isVacant ? "공실 등록" : "등록하기"}
              </button>
            </div>
          </div>
        </Modal>
      ...
      <div style={{ marginTop: 20, background: "linear-gradient(135deg,rgba(59,91,219,0.05),rgba(91,79,207,0.03))", border: "1px solid rgba(59,91,219,0.15)", borderRadius: 16, padding: "16px 20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          ...
          <button onClick={() => alert("🚧 세무사 연결 서비스 준비 중입니다.\n빠른 시일 내에 오픈할게요!")} style={{ padding: "9px 18px", borderRadius: 10, background: "linear-gradient(135deg,#3b5bdb,#5b4fcf)", border: "none", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>
            세무사 연결 →
          </button>
        </div>
      </div>
