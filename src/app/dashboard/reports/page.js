...
function ChartTab() {
  ...
  // ✅ 차트 데이터 없음 여부
  const hasChartData = monthlyData.some(d => d.income > 0 || d.missed > 0);
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", gap:8 }}>
        {[{ k:"3m", l:"3개월" }, { k:"6m", l:"6개월" }, { k:"1y", l:"1년" }].map(p => (
          <button key={p.k} onClick={() => setPeriod(p.k)} style={{ padding:"8px 18px", borderRadius:10, fontSize:13, fontWeight:700, cursor:"pointer", border:`2px solid ${period===p.k ? C.navy : C.border}`, background: period===p.k ? "rgba(26,39,68,0.07)" : "transparent", color: period===p.k ? C.navy : C.muted }}>{p.l}</button>
        ))}
      </div>
      ...
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:"20px" }}>
        ...
        {!hasChartData ? (
          <div style={{ height:140, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8, background:"rgba(26,39,68,0.02)", borderRadius:10 }}>
            <span style={{ fontSize:32 }}>📊</span>
            <p style={{ fontSize:13, color:C.muted, fontWeight:600 }}>아직 수금 데이터가 없어요</p>
            <p style={{ fontSize:11, color:"#c0c0cc" }}>수금 현황에서 납부 처리하면 차트가 표시됩니다</p>
            <button onClick={() => router.push("/dashboard/payments")} style={{ marginTop:4, padding:"6px 14px", borderRadius:8, background:C.navy, color:"#fff", border:"none", fontSize:11, fontWeight:700, cursor:"pointer" }}>수금 현황 가기 →</button>
          </div>
        ) : (
...
function PDFTab() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setRepairs(r.data || []); setLedger(l.data || []);
    };
    load();
  }, []);
  const handlePrint = () => {
    ...
    <div>
      <div className="no-print" style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          ...
          <button onClick={handlePrint} style={{ padding:"10px 22px", borderRadius:11, background:`linear-gradient(135deg,${C.navy},${C.purple})`, border:"none", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", gap:8 }}>🖨️ PDF 출력</button>
        </div>
      </div>
      <div id="report-print-area" ref={printRef}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
          <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
            ...
            {tenantSummary.length === 0 ? <p style={{ fontSize:13, color:C.muted }}>임차인 없음</p> : tenantSummary.map(t => (
          ...
          <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
            ...
            {Object.entries(expenseByCategory).length === 0 ? <p style={{ fontSize:13, color:C.muted }}>지출 내역 없음</p> : Object.entries(expenseByCategory).sort((a,b) => b[1]-a[1]).map(([cat,amt]) => (
  ...
  return (
    <div className="page-in page-padding" style={{ maxWidth:960 }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:28 }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{ padding:"10px 20px", borderRadius:11, fontSize:13, fontWeight:700, cursor:"pointer", border:`2px solid ${tab===t.key ? "#1a2744" : "#ebe9e3"}`, background: tab===t.key ? "rgba(26,39,68,0.07)" : "transparent", color: tab===t.key ? "#1a2744" : "#8a8a9a" }}>
            {t.label}
          </button>
        ))}
      </div>
