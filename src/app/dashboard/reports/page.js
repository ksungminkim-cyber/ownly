    ...
    <div className="page-in page-padding" style={{ maxWidth: 960 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 26, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "#fff", letterSpacing: "-.4px" }}>수익 리포트</h1>
          <p style={{ fontSize: 13, color: C.muted, marginTop: 3 }}>최근 6개월 · 순수익 <span style={{ color: C.emerald, fontWeight: 700 }}>{net}만원</span></p>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {[{ k: "3m", l: "3개월" }, { k: "6m", l: "6개월" }, { k: "1y", l: "1년" }].map((p) => (
            <button key={p.k} onClick={() => setPeriod(p.k)} style={{ padding: "6px 14px", borderRadius: 9, fontSize: 12, fontWeight: 600, cursor: "pointer", border: `1px solid ${period === p.k ? C.indigo : C.border}`, background: period === p.k ? C.indigo + "20" : "transparent", color: period === p.k ? C.indigo : C.muted }}>
              {p.l}
            </button>
          ))}
        </div>
      ...
      <div className="dash-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 18 }}>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 17, padding: "22px 24px" }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: C.text, marginBottom: 16 }}>월별 수입 vs 지출</h3>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 17, padding: "22px 24px" }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: C.text, marginBottom: 16 }}>순수익 추이</h3>
          <ResponsiveContainer width="100%" height={200}>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 17, overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${C.border}` }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: C.text }}>물건별 수익 분석</h3>
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {tenants.map((t) => {
              ...
              return (
                <tr key={t.id} className="trow" style={{ borderTop: `1px solid ${C.border}` }}>
                  <td style={{ padding: "12px 16px" }}>
                    ...
                    <p style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{t.addr}</p>
                  </td>
                  ...
                  <td style={{ padding: "12px 16px", fontSize: 12, color: C.muted }}>{(t.dep / 10000).toFixed(1)}억</td>
                  <td style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: yld !== "N/A" && parseFloat(yld) >= 5 ? C.emerald : C.amber }}>{yld}%</span>
                      <div style={{ width: 64, height: 4, borderRadius: 4, background: C.faint, overflow: "hidden" }}>
