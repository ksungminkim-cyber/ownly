...
function TaxContent() {
  ...
  const card = (label, value, color, sub) => (
    <div style={{ background: "#ffffff", border: "1px solid #ebe9e3", borderRadius: 14, padding: "18px 20px" }}>
      <p style={{ fontSize: 10, color: "#8a8a9a", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 8 }}>{label}</p>
      <p style={numStyle(value, color)}>{value}</p>
      {sub && <p style={{ fontSize: 11, color: "#8a8a9a", marginTop: 4 }}>{sub}</p>}
    </div>
  );
  ...
  const inp = (key, ph) => (
    <input
      ...
      placeholder={ph}
      style={{ width: "100%", padding: "10px 13px", fontSize: 13, color: "#1a2744", background: "#f8f7f4", border: "1px solid #ebe9e3", borderRadius: 9, outline: "none", boxSizing: "border-box" }}
    />
  );

  const lbl = (txt, hint) => (
    <div style={{ marginBottom: 7 }}>
      <p style={{ fontSize: 11, color: "#8a8a9a", fontWeight: 700, letterSpacing: ".5px", textTransform: "uppercase" }}>{txt}</p>
      {hint && <p style={{ fontSize: 11, color: "#8a8a9a", marginTop: 2 }}>{hint}</p>}
    </div>
  );
    ...
    <div className="page-in page-padding" style={{ padding: "36px 40px", maxWidth: 980 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
        {[{ k: "income", l: "📊 종합소득세" }, { k: "vat", l: "🧾 부가가치세" }, { k: "summary", l: "📋 세금 요약" }].map((t) => (
          <button key={t.k} onClick={() => setTab(t.k)}
            style={{ padding: "10px 20px", borderRadius: 11, fontSize: 13, fontWeight: 700, cursor: "pointer", border: `2px solid ${tab === t.k ? C.indigo : "#ebe9e3"}`, background: tab === t.k ? C.indigo + "18" : "transparent", color: tab === t.k ? C.indigo : C.muted }}>
            {t.l}
          </button>
        ))}
      </div>
      ...
      {tab === "income" && (
        <div className="tax-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {/* 임대 수입 요약 */}
            <div style={{ background: "#ffffff", border: "1px solid #ebe9e3", borderRadius: 14, padding: "18px 20px" }}>
              <p style={{ fontSize: 11, color: "#1a2744", fontWeight: 700, letterSpacing: ".5px", textTransform: "uppercase", marginBottom: 14 }}>임대 수입 (자동 계산)</p>
              {allTenants.length === 0 ? (
                <p style={{ fontSize: 13, color: "#8a8a9a" }}>등록된 세입자가 없습니다</p>
              ) : allTenants.map((t) => (
                <div key={t.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #ebe9e3" }}>
                  <span style={{ fontSize: 13, color: "#1a2744" }}>{t.name} <span style={{ fontSize: 11, color: "#8a8a9a" }}>({t.addr})</span></span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#0fa573" }}>{(t.rent * 12).toLocaleString()}만원/년</span>
                </div>
              ))}
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#1a2744" }}>연간 임대 수입 합계</span>
                <span style={{ fontSize: 15, fontWeight: 800, color: "#1a2744" }}>{annualRent.toLocaleString()}만원</span>
              </div>
            </div>

            {/* 임대 외 소득 */}
            <div style={{ background: "#ffffff", border: "1px solid #ebe9e3", borderRadius: 14, padding: "18px 20px" }}>
              <p style={{ fontSize: 11, color: "#e8960a", fontWeight: 700, letterSpacing: ".5px", textTransform: "uppercase", marginBottom: 12 }}>임대 외 소득 (만원/년)</p>
            ...
            {/* 필요경비 */}
            <div style={{ background: "#ffffff", border: "1px solid #ebe9e3", borderRadius: 14, padding: "18px 20px" }}>
              <p style={{ fontSize: 11, color: "#0fa573", fontWeight: 700, letterSpacing: ".5px", textTransform: "uppercase", marginBottom: 14 }}>필요경비 공제 (만원/년)</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                ...
                <div style={{ borderTop: "1px solid #ebe9e3", paddingTop: 10, display: "flex", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 13, color: "#8a8a9a" }}>총 공제액</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: "#0fa573" }}>{totalDeductAmt.toLocaleString()}만원</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ background: `linear-gradient(135deg,${C.rose}18,${C.purple}18)`, border: `1px solid ${C.rose}30`, borderRadius: 16, padding: "22px" }}>
              <p style={{ fontSize: 11, color: "#8a8a9a", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 10 }}>총 납부 세액 추정</p>
              <p style={{ fontSize: 32, fontWeight: 900, color: "#e8445a" }}>{totalTax.toLocaleString()}만원</p>
              <p style={{ fontSize: 13, color: "#8a8a9a", marginTop: 6 }}>세후 임대 수입: <span style={{ color: "#0fa573", fontWeight: 700 }}>{afterTax.toLocaleString()}만원/년</span></p>
            </div>

            {/* 세율 구간 시각화 */}
            <div style={{ background: "#ffffff", border: "1px solid #ebe9e3", borderRadius: 14, padding: "18px 20px" }}>
              <p style={{ fontSize: 11, color: "#8a8a9a", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 14 }}>소득세 세율 구간</p>
              {[
                ...
                return (
                  <div key={range} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: "1px solid #ebe9e3" }}>
                    <div style={{ width: 8, height: 8, borderRadius: 2, background: isActive ? C.rose : "#ebe9e3", flexShrink: 0 }} />
            ...
            <div style={{ background: "#f8f7f4", border: "1px solid #ebe9e3", borderRadius: 12, padding: "14px 16px" }}>
              <p style={{ fontSize: 12, color: "#8a8a9a", lineHeight: 1.7 }}>
                ⚠️ 본 시뮬레이션은 <strong style={{ color: "#1a2744" }}>참고용 추정치</strong>입니다.<br />
      ...
      {tab === "vat" && (
        <div style={{ maxWidth: 640 }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            {[{ k: "1h", l: "1기 (1~6월)" }, { k: "2h", l: "2기 (7~12월)" }].map((p) => (
              <button key={p.k} onClick={() => setPeriod(p.k)}
                style={{ padding: "7px 16px", borderRadius: 9, fontSize: 12, fontWeight: 600, cursor: "pointer", border: `1px solid ${period === p.k ? C.indigo : "#ebe9e3"}`, background: period === p.k ? C.indigo + "20" : "transparent", color: period === p.k ? C.indigo : C.muted }}>
                {p.l}
              </button>
            ))}
          </div>

          <div style={{ background: "#ffffff", border: "1px solid #ebe9e3", borderRadius: 14, padding: "22px", marginBottom: 16 }}>
            <p style={{ fontSize: 11, color: "#e8960a", fontWeight: 700, letterSpacing: ".5px", textTransform: "uppercase", marginBottom: 14 }}>과세 대상 (상가 임대)</p>
            {vatTenants.length === 0 ? (
              <p style={{ fontSize: 13, color: "#8a8a9a" }}>등록된 상가 세입자가 없습니다</p>
            ) : vatTenants.map((t) => (
              <div key={t.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #ebe9e3" }}>
                <span style={{ fontSize: 13, color: "#1a2744" }}>{t.name} <span style={{ fontSize: 11, color: "#8a8a9a" }}>({t.addr})</span></span>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#1a2744" }}>{(t.rent * 6).toLocaleString()}만원 ({period === "1h" ? "1기" : "2기"})</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "#f8f7f4", border: "1px solid #ebe9e3", borderRadius: 12, padding: "14px 16px" }}>
            <p style={{ fontSize: 12, color: "#8a8a9a", lineHeight: 1.7 }}>
              📌 <strong style={{ color: "#1a2744" }}>부가세 신고 일정</strong><br />
      ...
      {tab === "summary" && (
        <div style={{ maxWidth: 640 }}>
          <div style={{ background: "#ffffff", border: "1px solid #ebe9e3", borderRadius: 16, overflow: "hidden", marginBottom: 16 }}>
            <div style={{ padding: "18px 22px", borderBottom: "1px solid #ebe9e3", background: "#f8f7f4" }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#1a2744" }}>연간 세금 요약</p>
            </div>
            {[
              ...
              <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "14px 22px", borderBottom: "1px solid #ebe9e3" }}>
                <span style={{ fontSize: 13, color: "#8a8a9a" }}>{l}</span>
                <span style={{ fontSize: 14, fontWeight: 700, color: c }}>{v}</span>
              </div>
            ))}
            <div style={{ display: "flex", justifyContent: "space-between", padding: "18px 22px", background: "#f8f7f4" }}>
              <span style={{ fontSize: 14, fontWeight: 800, color: "#1a2744" }}>실수령 추정</span>
              <span style={{ fontSize: 18, fontWeight: 900, color: "#0fa573" }}>{(annualRent - totalTax - vatAmount * 2).toLocaleString()}만원</span>
            </div>
          </div>
