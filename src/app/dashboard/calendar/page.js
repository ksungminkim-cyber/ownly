"use client"; import { useState, useMemo, useEffect } from "react"; import { SectionLabel } from "../../../components/shared"; import { C } from "../../../lib/constants"; import { useApp } from "../../../context/AppContext"; const MONTH_KO = ["1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"]; const DAY_KO = ["일","월","화","수","목","금","토"];
...
export default function CalendarPage() {
  ...
  const getPayDayForMonth = (t, yr, mo) => {
    ...
    if (!pd || pd === 0) return 1;
    if (pd === 99) return new Date(yr, mo + 1, 0).getDate();
  ...
  const eventMap = useMemo(() => {
    ...
    tenants.filter(t => t.status !== "공실").forEach((t) => {
      ...
      // ✅ ⑩ 보증금 반환 예정 — 계약 만료 당일에 표시
      if (t.end_date && t.dep && t.dep > 0) {
        const d = new Date(t.end_date);
        if (d.getFullYear() === year && d.getMonth() === month)
          add(d.getDate(), {
            type: "deposit_return",
            label: t.name,
            sub: `보증금 ${(t.dep / 10000).toFixed(1)}억 반환`,
            color: "#7c3aed",
            amount: t.dep,
          });
      }
  ...
  // ✅ ⑩ 향후 3개월 내 보증금 반환 예정 목록 (사이드바 상단)
  const upcomingDeposits = useMemo(() => {
    const result = [];
    const now = new Date();
    const future = new Date(now);
    future.setMonth(future.getMonth() + 3);
    tenants.filter(t => t.end_date && t.dep > 0 && t.status !== "공실").forEach(t => {
      const end = new Date(t.end_date);
      if (end >= now && end <= future) {
        const diffDays = Math.ceil((end - now) / 86400000);
        result.push({ ...t, end, diffDays });
      }
    });
  ...
  const allEvents = useMemo(() => {
    return Object.entries(eventMap).sort(([a], [b]) => Number(a) - Number(b))
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflowY: "auto", padding: "28px 28px 28px 36px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 3, flex: 1 }}>
          {cells.map((d, i) => {
            ...
            const col = i % 7;
            const hasDeposit = evs.some(e => e.type === "deposit_return");
            return (
              <div key={d} onClick={() => setSelected(isSel ? null : d)} style={{ minHeight: 72, borderRadius: 10, padding: "8px 7px", cursor: "pointer", background: isSel ? C.indigo + "25" : isToday(d) ? C.indigo + "12" : "#ffffff", border: `1px solid ${isSel ? C.indigo : isToday(d) ? C.indigo + "60" : "#ebe9e3"}`, transition: "all .12s", position: "relative" }}>
                {/* ✅ 보증금 반환 있으면 보라색 점 */}
                {hasDeposit && <span style={{ position: "absolute", top: 5, right: 7, width: 7, height: 7, borderRadius: "50%", background: "#7c3aed" }} />}
                <div style={{ fontSize: 13, fontWeight: isToday(d) ? 800 : 600, color: isToday(d) ? C.indigo : col === 0 ? C.rose : col === 6 ? "#818cf8" : "#1a2744", marginBottom: 4 }}>
      ...
      {/* 사이드바 */}
      <div style={{ width: 280, borderLeft: "1px solid #ebe9e3", overflowY: "auto", padding: "28px 18px", flexShrink: 0 }}>

        {/* ✅ ⑩ 보증금 반환 임박 섹션 */}
        {upcomingDeposits.length > 0 && (
          <div style={{ marginBottom: 22 }}>
            <p style={{ fontSize: 10, color: "#7c3aed", fontWeight: 800, letterSpacing: "1px", textTransform: "uppercase", marginBottom: 10 }}>💜 보증금 반환 예정 (3개월 내)</p>
            {upcomingDeposits.map(t => (
              <div key={t.id} style={{ background: "rgba(124,58,237,0.06)", border: "1px solid rgba(124,58,237,0.2)", borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#1a2744" }}>{t.name}</span>
                  <span style={{ fontSize: 11, fontWeight: 800, color: t.diffDays <= 14 ? "#e8445a" : "#7c3aed" }}>D-{t.diffDays}</span>
                </div>
                <p style={{ fontSize: 11, color: "#8a8a9a", marginBottom: 4 }}>{t.addr}</p>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: "#7c3aed" }}>💰 {(t.dep / 10000).toFixed(1)}억 반환</span>
                  <span style={{ fontSize: 10, color: "#8a8a9a" }}>{t.end_date?.slice(0, 10)}</span>
                </div>
                {t.diffDays <= 14 && (
                  <div style={{ marginTop: 6, padding: "4px 8px", background: "rgba(232,68,90,0.08)", borderRadius: 6, fontSize: 10, color: "#e8445a", fontWeight: 700 }}>
                    ⚠️ 2주 내 반환 필요 — 잔금 확인하세요
                  </div>
                )}
              </div>
            ))}
            ...
            {selectedEvents.length === 0
              ...
              : selectedEvents.map((ev, i) => (
                <div key={i} style={{ background: "#ffffff", border: `1px solid ${ev.color}30`, borderLeft: `3px solid ${ev.color}`, borderRadius: 10, padding: "11px 12px", marginBottom: 8 }}>
                  ...
                  {ev.type === "deposit_return" && (
                    <div style={{ marginTop: 6, fontSize: 12, fontWeight: 800, color: "#7c3aed" }}>💰 {(ev.amount / 10000).toFixed(1)}억원 반환 예정</div>
                  )}
                </div>
              ))
            }
            ...
            {allEvents.length === 0
              ...
              : allEvents.map((ev, i) => (
                <div key={i} onClick={() => setSelected(ev.date)} style={{ background: "#ffffff", border: "1px solid #ebe9e3", borderLeft: `3px solid ${ev.color}`, borderRadius: 10, padding: "10px 12px", marginBottom: 7, cursor: "pointer" }}>
                  ...
                  <div style={{ fontSize: 13, color: "#1a2744", fontWeight: 600 }}>{ev.label}</div>
                  {ev.type === "deposit_return" && (
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#7c3aed", marginTop: 3 }}>💰 {(ev.amount / 10000).toFixed(1)}억 반환</div>
                  )}
                </div>
              ))
            }
