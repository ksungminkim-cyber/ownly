"use client"; import { useState, useEffect, useRef } from "react"; import { SectionLabel, toast } from "../../../components/shared"; import { C } from "../../../lib/constants"; import { useApp } from "../../../context/AppContext"; import { supabase } from "../../../lib/supabase"; import { generateNickname } from "../../../lib/nickname";
...
export default function CommunityPage() {
  ...
  const loadPosts = async () => { setLoading(true); let q = supabase.from("community_posts").select("*").order("created_at", { ascending: false }); if (category !== "전체") q = q.eq("category", category); const { data } = await q; setPosts(data || []); setLoading(false); };
  const loadMyLikes = async () => { const { data } = await supabase.from("community_likes").select("post_id").eq("user_id", user.id); setMyLikes(new Set((data||[]).map(l => l.post_id))); };
  ...
  const deletePost = async (id) => { await supabase.from("community_posts").delete().eq("id", id); setPosts(prev => prev.filter(p => p.id !== id)); closePanel(); toast("삭제되었습니다"); };
    ...
    <div style={{ fontFamily:"'Pretendard','DM Sans',sans-serif", height:"calc(100vh - 60px)", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <div style={{ padding:"20px 28px 0", flexShrink:0 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", flexWrap:"wrap", gap:10, marginBottom:14 }}>
          ...
          <button
            onClick={() => { setForm({ title:"", content:"", category: category !== "전체" ? category : "자유", author_name: myNickname }); setImages([]); setPreviews([]); setShowWrite(true); }}
        ...
        <div style={{ display:"flex", gap:5, flexWrap:"wrap", paddingBottom:14, borderBottom:"1px solid var(--border)" }}>
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setCategory(c)} style={{
              ...
              border:`1px solid ${category===c ? (c===CAT_HIGHLIGHT?"#0fa573":C.indigo) : c===CAT_HIGHLIGHT?"rgba(15,165,115,0.4)":"var(--border)"}`,
              background: category===c ? (c===CAT_HIGHLIGHT?"#0fa573":C.indigo) : c===CAT_HIGHLIGHT?"rgba(15,165,115,0.07)":"transparent",
              color: category===c ? "#fff" : c===CAT_HIGHLIGHT?"#0fa573":"var(--text-muted)"
            }}>
              {c===CAT_HIGHLIGHT ? "🏠 "+c : c}
            </button>
          ))}
        ...
        <div style={{ flex: panelOpen && !isMobile ? "0 0 380px" : "1", overflowY:"auto", borderRight: panelOpen && !isMobile ? "1px solid var(--border)" : "none", transition:"flex .3s ease" }}>
          {loading ? (
            ...
            <div style={{ textAlign:"center", padding:60 }}>
              ...
              {category===CAT_HIGHLIGHT && <p style={{ fontSize:12, color:"var(--text-muted)", marginTop:6 }}>글쓰기 버튼을 눌러 공실 물건을 올려보세요</p>}
            </div>
          ) : (
            <div>
              {posts.map(post => {
        ...
        {activePost && (
          <div style={{ ...(isMobile ? { position:"fixed", top:0, right:0, bottom:0, width:"100%", zIndex:300, transform:panelOpen?"translateX(0)":"translateX(100%)" } : { flex:1, position:"relative", transform:panelOpen?"translateX(0)":"translateX(20px)", opacity:panelOpen?1:0 }), background:"var(--surface)", overflowY:"auto", transition:"transform .3s ease, opacity .3s ease", display:"flex", flexDirection:"column" }}>
            <div style={{ position:"sticky", top:0, zIndex:10, background:"var(--surface)", borderBottom:"1px solid var(--border)", padding:"13px 20px", display:"flex", alignItems:"center", gap:10 }}>
              ...
              {activePost.user_id === user?.id && <button onClick={() => deletePost(activePost.id)} style={{ marginLeft:"auto", fontSize:11, padding:"4px 10px", borderRadius:7, border:`1px solid ${C.rose}`, background:"transparent", color:C.rose, cursor:"pointer" }}>삭제</button>}
            </div>
            <div style={{ flex:1, padding:"20px 24px", overflowY:"auto" }}>
              ...
              <div style={{ display:"flex", gap:14, paddingBottom:16, borderBottom:"1px solid var(--border)", marginBottom:16 }}>
                ...
                <button onClick={() => toggleLike(activePost.id)} style={{ fontSize:12, background:"none", border:"none", cursor:"pointer", fontWeight:700, color:myLikes.has(activePost.id)?C.rose:"var(--text-muted)" }}>{myLikes.has(activePost.id)?"❤️":"🤍"} {activePost.likes} 좋아요</button>
              </div>
            ...
            <div style={{ padding:"12px 16px", borderTop:"1px solid var(--border)", background:"var(--surface)", flexShrink:0 }}>
              <div style={{ display:"flex", gap:8 }}>
                <input value={newComment} onChange={e => setNewComment(e.target.value)} onKeyDown={e => e.key==="Enter" && !e.shiftKey && submitComment()} placeholder="댓글을 입력하세요..." style={{ flex:1, padding:"10px 13px", fontSize:13, color:"var(--text)", background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:10, outline:"none" }} />
        ...
        <div style={{ position:"fixed", inset:0, zIndex:400, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }} onClick={() => setShowWrite(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background:"var(--surface)", borderRadius:18, padding:24, width:"100%", maxWidth:520, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.25)" }}>
            ...
            <div style={{ marginBottom:12 }}>
              ...
              <input value={form.author_name} onChange={e => setForm(f => ({...f, author_name:e.target.value}))} maxLength={20} style={{ width:"100%", padding:"9px 12px", fontSize:13, color:"var(--text)", background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:10, outline:"none", boxSizing:"border-box" }} />
            </div>
            <div style={{ marginBottom:12 }}>
              <p style={{ fontSize:11, fontWeight:700, color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:".5px", marginBottom:6 }}>제목</p>
              <input value={form.title} onChange={e => setForm(f => ({...f, title:e.target.value}))} placeholder="제목을 입력하세요" style={{ width:"100%", padding:"9px 12px", fontSize:13, color:"var(--text)", background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:10, outline:"none", boxSizing:"border-box" }} />
            </div>
            <div style={{ marginBottom:14 }}>
              <p style={{ fontSize:11, fontWeight:700, color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:".5px", marginBottom:6 }}>내용</p>
              <textarea value={form.content} onChange={e => setForm(f => ({...f, content:e.target.value}))} placeholder="내용을 입력하세요..." rows={5} style={{ width:"100%", padding:"10px 12px", fontSize:13, color:"var(--text)", background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:10, resize:"vertical", outline:"none", lineHeight:1.7, boxSizing:"border-box" }} />
            </div>
            <div style={{ marginBottom:18 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                ...
                <button onClick={() => fileInputRef.current?.click()} disabled={images.length>=3} style={{ fontSize:12, color:images.length>=3?"var(--text-muted)":C.indigo, fontWeight:700, background:"none", border:"none", cursor:images.length>=3?"not-allowed":"pointer" }}>📷 사진 추가</button>
              </div>
