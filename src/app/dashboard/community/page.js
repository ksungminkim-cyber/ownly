...
import { generateNickname, getNickname } from "../../../lib/nickname";
...
export default function CommunityPage() {
  ...
  // 닉네임: user_metadata.nickname 우선, 없으면 세션 내 자동 생성
  ...
  const submitComment = async () => {
    ...
    const row = { post_id: showDetail.id, user_id: user.id, author_name: myNickname, content: newComment.trim() };
  ...
  const toggleLike = async (postId) => {
    ...
    if (liked) {
      ...
      await supabase.from("community_posts").update({ likes: (post?.likes || 1) - 1 }).eq("id", postId);
    ...
    <div className="page-in page-padding" style={{ maxWidth: 860 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:20, flexWrap:"wrap", gap:12 }}>
        <div>
          ...
          <h1 style={{ fontSize:24, fontWeight:800, color:"var(--text)" }}>임대인 커뮤니티</h1>
          {myBadge && (
        ...
        <button onClick={() => { setForm({ title:"", content:"", category:"자유", author_name: myNickname }); setShowWrite(true); }}
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:18 }}>
        {CATEGORIES.map(c => (
          <button key={c} onClick={() => setCategory(c)}
            style={{ padding:"5px 13px", borderRadius:20, fontSize:12, fontWeight:600, cursor:"pointer", border:`1px solid ${category===c?C.indigo:"var(--border)"}`, background:category===c?C.indigo:"transparent", color:category===c?"#fff":C.muted }}>
            {c}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:"center", padding:60 }}>
          ...
          <p style={{ fontSize:15, fontWeight:700, color:"var(--text)" }}>첫 번째 글을 작성해보세요!</p>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {posts.map(post => (
            <div key={post.id} onClick={() => openPost(post)} className="hover-lift"
              style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"16px 20px", cursor:"pointer" }}>
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                    ...
                    {post.badge_label && (
                      <span style={{ fontSize:9, fontWeight:700, color:post.badge_color||"#8a8a9a", background:post.badge_bg||"#f0efe9", padding:"1px 6px", borderRadius:20 }}>{post.badge_label}</span>
                    )}
      ...
      <Modal open={showWrite} onClose={() => setShowWrite(false)}>
        <h2 style={{ fontSize:18, fontWeight:800, color:"var(--text)", marginBottom:16 }}>글쓰기</h2>
        <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              {CATEGORIES.slice(1).map(c => (
                <button key={c} onClick={() => setForm(f=>({...f,category:c}))}
                  style={{ padding:"4px 11px", borderRadius:8, fontSize:12, fontWeight:600, cursor:"pointer", border:`1px solid ${form.category===c?C.indigo:"var(--border)"}`, background:form.category===c?C.indigo+"15":"transparent", color:form.category===c?C.indigo:C.muted }}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:7 }}>
              <p style={{ fontSize:11, color:C.muted, fontWeight:700, textTransform:"uppercase", letterSpacing:".5px" }}>닉네임</p>
            ...
            <p style={{ fontSize:10, color:C.muted, marginTop:4 }}>설정에서 기본 닉네임을 변경할 수 있어요</p>
          </div>
          <div>
          ...
          <div>
            ...
            <textarea value={form.content} onChange={e=>setForm(f=>({...f,content:e.target.value}))} placeholder="내용을 입력하세요..." rows={6}
              style={{ width:"100%", padding:"11px 13px", fontSize:13, color:"var(--text)", background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:10, resize:"vertical", outline:"none", lineHeight:1.7 }} />
          </div>
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={()=>setShowWrite(false)} style={{ flex:1, padding:12, borderRadius:11, background:"transparent", border:"1px solid var(--border)", color:C.muted, fontWeight:600, fontSize:13, cursor:"pointer" }}>취소</button>
            <button onClick={submitPost} style={{ flex:2, padding:12, borderRadius:11, background:`linear-gradient(135deg,${C.indigo},${C.purple})`, border:"none", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer" }}>등록하기</button>
          </div>
        </div>
      </Modal>
        ...
        <Modal open={!!showDetail} onClose={()=>setShowDetail(null)}>
          <div style={{ maxHeight:"75vh", overflowY:"auto" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:"var(--text)" }}>{showDetail.author_name}</span>
              {showDetail.badge_label && (
                <span style={{ fontSize:9, fontWeight:700, color:showDetail.badge_color||"#8a8a9a", background:showDetail.badge_bg||"#f0efe9", padding:"1px 7px", borderRadius:20 }}>{showDetail.badge_label}</span>
              )}
            ...
            <h2 style={{ fontSize:18, fontWeight:800, color:"var(--text)", marginBottom:14 }}>{showDetail.title}</h2>
            <p style={{ fontSize:14, color:"var(--text)", lineHeight:1.8, whiteSpace:"pre-wrap", marginBottom:18 }}>{showDetail.content}</p>
            <div style={{ display:"flex", gap:14, paddingBottom:16, borderBottom:"1px solid var(--border)" }}>
            ...
            <div style={{ marginTop:16 }}>
              ...
              {comments.map(c => (
                <div key={c.id} style={{ background:"var(--surface2)", borderRadius:10, padding:"11px 14px", marginBottom:8 }}>
              ...
              <div style={{ display:"flex", gap:8, marginTop:12 }}>
                <input value={newComment} onChange={e=>setNewComment(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submitComment()} placeholder="댓글을 입력하세요..."
