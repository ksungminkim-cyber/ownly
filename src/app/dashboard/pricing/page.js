...
export default function PricingPage() {
  ...
  const COMPARE_ROWS = [
    ...
    { label: "📊 공실 손실",      vals: [false, false, true, true] },
    { label: "📋 임대차 3법",     vals: [false, false, true, true] },
    { label: "🗺️ 매물 조회",      vals: [false, false, false, true] },
    { label: "🤖 AI 분석",        vals: [false, false, false, true] },
  ...
  return (
    <div style={{ fontFamily: "'Pretendard','DM Sans',sans-serif", maxWidth: 1060, margin: "0 auto", padding: isMobile ? "24px 16px 48px" : "36px 24px 60px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: isMobile ? 28 : 48 }}>
        ...
        <h1 style={{ fontSize: isMobile ? 22 : 32, fontWeight: 900, color: "var(--text)", lineHeight: 1.2, margin: "0 0 10px" }}>
          나에게 맞는 플랜을 선택하세요
        </h1>
        <p style={{ color: "#8a8a9a", fontSize: 13 }}>언제든지 업그레이드 · 다운그레이드 가능 · VAT 포함</p>
      </div>

      {/* 플랜 카드 — 모바일: 세로 스크롤 / 데스크탑: 4열 */}
      {isMobile ? (
        // 모바일: 가로 스크롤 카드
          ...
          {planList.map((plan) => {
            ...
            return (
              <div key={plan.id} style={{
                background: "var(--surface)", border: `2px solid ${isCurrent ? th.accent : "var(--border)"}`,
                borderRadius: 18, padding: "20px 16px", minWidth: 220, maxWidth: 240,
                flexShrink: 0, scrollSnapAlign: "start", position: "relative",
                display: "flex", flexDirection: "column",
                boxShadow: isCurrent ? th.shadow : "0 2px 10px rgba(26,39,68,0.06)",
              }}>
                {plan.badge && (
                ...
                <button onClick={() => handleUpgrade(plan.id)} disabled={isCurrent || !!loading}
                  style={{ width: "100%", padding: "10px", borderRadius: 10, border: "none", cursor: isCurrent ? "default" : "pointer",
                    fontWeight: 800, fontSize: 12, background: isCurrent ? "#f0efe9" : th.btnBg, color: isCurrent ? "#8a8a9a" : th.btnColor }}>
          ...
          {planList.map((plan) => {
            ...
            return (
              <div key={plan.id} style={{
                ...
                boxShadow: isCurrent ? th.shadow : isHighlighted ? "0 8px 32px rgba(15,165,115,0.15)" : "0 2px 10px rgba(26,39,68,0.04)",
                ...
                <button onClick={() => handleUpgrade(plan.id)} disabled={isCurrent || !!loading}
                  style={{ width: "100%", padding: "12px", borderRadius: 12, border: "none",
                    cursor: isCurrent || loading ? "default" : "pointer", fontWeight: 800, fontSize: 13,
                    opacity: loading && loading !== plan.id ? 0.5 : 1,
                    background: isCurrent ? "#f0efe9" : th.btnBg, color: isCurrent ? "#8a8a9a" : th.btnColor,
                    boxShadow: isCurrent ? "none" : th.shadow }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto 48px" }}>
        ...
        {FAQS.map((faq, i) => (
          <div key={i} onClick={() => setFaqOpen(faqOpen === i ? null : i)}
            style={{ marginBottom: 10, background: "var(--surface)", borderRadius: 14,
              border: `1px solid ${faqOpen === i ? "#1a2744" : "var(--border)"}`, overflow: "hidden", cursor: "pointer" }}>
      ...
      {/* 하단 CTA */}
      <div style={{ textAlign: "center", padding: isMobile ? "24px 16px" : "36px",
        background: "linear-gradient(135deg,rgba(26,39,68,0.04),rgba(15,165,115,0.04))",
        ...
        <button onClick={() => router.push("/dashboard")}
          style={{ padding: "12px 28px", borderRadius: 12, background: "#1a2744", color: "#fff", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700 }}>
          대시보드로 이동 →
        </button>
      </div>
    </div>
