...
const C = {
  ...
  amber:"#e8960a", indigo:"#3b5bdb", border:"#e8e6e0", surface:"#ffffff",
  faint:"#f8f7f4", muted:"#8a8a9a"
};
...
export default function ReportPDFPage() {
  ...
  // ✅ 임차인별 수금 현황 — 버그 수정
  // 기존: expectedAmt = rent * 12 (연간 전체 대비) → 실제 납부 개월이 적으면 항상 낮게 나옴
  // 수정: 선택 연도 내 실제 납부된 개월 수 / 계약 유효 개월 수 기준으로 계산
  ...
  const tenantSummary = tenants.map(t => {
    ...
    // 계약 유효 개월: 해당 연도 기준, 최대 현재 월까지만 계산
    ...
    <div className="page-in page-padding" style={{ maxWidth: 900 }}>
      ...
      <div className="no-print" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 26, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button onClick={handlePrint} style={{ padding: "10px 22px", borderRadius: 11, background: `linear-gradient(135deg,${C.navy},${C.purple})`, border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
            🖨️ PDF 출력
          </button>
        </div>
      </div>
              ...
              : tenantSummary.map(t => (
                <div key={t.id} style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3 }}>
                    ...
                    <span style={{ fontSize: 10, color: C.muted }}>{t.paidMonths}개월 납부 / {t.validMonths}개월 기준</span>
                  </div>
                </div>
              ))
            }
