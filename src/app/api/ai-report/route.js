...
export async function POST(req) {
  try {
    ...
    const prompt = buildPrompt(address, propertyType || "주거");
    ...
    const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "당신은 대한민국 부동산 입지 분석 전문가입니다. 반드시 순수한 한국어로만 응답하세요. 한자(漢字), 영어, 베트남어, 일본어, 스페인어 등 어떤 외국어 단어도 절대 섞지 마세요. 예를 들어 '附近', '地域', '利用', '主要' 같은 한자와 'sát', 'của' 같은 외래어를 절대 사용하지 마세요. 모든 단어는 한글로만 작성하세요. 요청된 JSON 형식으로만 응답하고, 각 분석 항목은 반드시 구체적인 수치와 지역 특성을 포함해 150자 이상 작성하세요. 문자열 값 안에 줄바꿈 문자를 포함하지 마세요.",
          },
