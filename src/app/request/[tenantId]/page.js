...
export default function RepairRequestPage() {
  ...
  const submit = async () => {
    ...
    setErrorMsg("");
    try {
      const memo = form.urgent ? "[\uAE34\uAE09] " + form.desc : form.desc;
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#f5f4f0", fontFamily:"'Pretendard',sans-serif" }}>
      <div style={{ textAlign:"center" }}><div style={{ fontSize:40, marginBottom:16 }}>&#9203;</div><p style={{ fontSize:14, color:"#8a8a9a" }}>\uBD88\uB7EC\uC624\uB294 \uC911...</p></div>
    </div>
  );
  ...
  if (done) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#f5f4f0", fontFamily:"'Pretendard',sans-serif", padding:20 }}>
      <div style={{ textAlign:"center", maxWidth:360 }}>
        ...
        <p style={{ fontSize:14, color:"#8a8a9a", lineHeight:1.7, marginBottom:24 }}>\uC784\uB300\uC778\uC5D0\uAC8C \uC54C\uB9BC\uC774 \uC804\uC1A1\uB418\uC5C8\uC2B5\uB2C8\uB2E4.<br/>\uBE60\uB978 \uC2DC\uC77C \uB0B4\uC5D0 \uC5F0\uB77D \uB4DC\uB9B4\uAC8C\uC694.</p>
    ...
    <div style={{ minHeight:"100vh", background:"#f5f4f0", fontFamily:"'Pretendard','DM Sans',sans-serif", padding:"0 0 40px" }}>
      <div style={{ background:"#1a2744", padding:"20px 20px 24px" }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          ...
          <p style={{ fontSize:13, color:"rgba(255,255,255,0.6)" }}>{tenant?.address || ""} &middot; {tenant?.name || ""}</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth:480, margin:"0 auto", padding:"24px 20px" }}>
            ...
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setForm(f=>({...f, category:cat}))}
                style={{ padding:"10px 4px", borderRadius:10, fontSize:12, fontWeight:600, cursor:"pointer", textAlign:"center",
                  border:`1.5px solid ${form.category===cat?"#1a2744":"#ebe9e3"}`,
                  background:form.category===cat?"#1a2744":"transparent",
                  color:form.category===cat?"#fff":"#4a5568", transition:"all .15s" }}>
        ...
        <div style={{ background:"#fff", borderRadius:16, padding:"16px 20px", marginBottom:20, border:"1px solid #ebe9e3" }}>
          <div onClick={() => setForm(f=>({...f, urgent:!f.urgent}))}
            ...
            <div style={{ width:22, height:22, borderRadius:6,
              border:`2px solid ${form.urgent?"#e8445a":"#ebe9e3"}`,
              background:form.urgent?"#e8445a":"transparent",
        ...
        {errorMsg && (
          <div style={{ background:"rgba(232,68,90,0.08)", border:"1px solid rgba(232,68,90,0.3)", borderRadius:10, padding:"10px 14px", marginBottom:14 }}>
            <p style={{ fontSize:12, color:"#e8445a", margin:0 }}>&#9888; {errorMsg}</p>
          </div>
        )}
        ...
        <button
          ...
          style={{ width:"100%", padding:"16px", borderRadius:14,
            background: !form.desc.trim() ? "#e0e0e0" : form.urgent ? "linear-gradient(135deg,#e8445a,#dc2626)" : "linear-gradient(135deg,#1a2744,#2d4270)",
            border:"none", color: !form.desc.trim() ? "#aaa" : "#fff",
            fontWeight:800, fontSize:16,
            cursor: form.desc.trim() && !submitting ? "pointer" : "not-allowed",
            boxShadow: form.desc.trim() ? "0 4px 20px rgba(26,39,68,0.25)" : "none" }}
        >
