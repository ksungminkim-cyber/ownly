...
export function Sidebar({ onLogout }) {
  ...
  return (
    <aside className="desktop-sidebar" style={{ width: 230, height: "100vh", background: "var(--sidebar-bg)", borderRight: "1px solid var(--sidebar-border)", position: "fixed", top: 0, left: 0, display: "flex", flexDirection: "column", zIndex: 100, overflow: "hidden", boxShadow: "2px 0 20px var(--shadow)" }}>
      {/* 로고 */}
      ...
      <nav style={{ flex: 1, overflowY: "auto", padding: "0 10px 8px" }}>
        ...
        {docsOpen && (
          <div style={{ animation: "sb-fade .15s ease" }}>
            ...
            {(() => { const ok = PLAN_ORDER[userPlan || "free"] >= PLAN_ORDER["plus"]; return (
              <div onClick={() => router.push(ok ? "/dashboard/reports" : "/dashboard/pricing")} className={"nav-item" + (pathname.includes("/dashboard/reports") ? " active" : "")}
                style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"6px 10px", borderRadius:10, marginBottom:1, cursor:"pointer", borderLeft:`2.5px solid ${pathname.includes("/dashboard/reports") ? "#5b4fcf" : "transparent"}`, opacity: ok ? 1 : 0.45 }}>
                <div style={{ display:"flex", alignItems:"center", gap:9 }}>
                  <span style={{ fontSize:15 }}>📊</span>
